...
export const foiaStatutes = pgTable("foia_statutes", {
  id: serial("id").primaryKey(),
  stateCode: varchar("state_code", { length: 8 }).notNull(),
  jurisdictionLevel: text("jurisdiction_level"),
  lawName: text("statute_name").notNull(),
  statuteReference: text("citation").notNull(),
  responseDeadlineDays: integer("response_days"),
  responseDeadlineUnit: text("response_days_unit"),
  extensionRule: text("extension_rule"),
  appealDeadlineDays: integer("appeal_deadline_days"),
  appealNote: text("appeal_note"),
  feeWaiverAvailable: boolean("fee_waiver_available").default(true).notNull(),
  expeditedProcessingAvailable: boolean("expedited_processing_available").default(false).notNull(),
  officialUrl: text("official_url"),
  sourceHash8: text("source_hash8"),
  notes: text("notes"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [
...
export const foiaAgencies = pgTable("foia_agencies", {
  ...
  agencyComponent: text("component_name"),
  jurisdictionLevel: text("jurisdiction_level").$type<"federal" | "state" | "county" | "municipal" | "court">(),
  mailingAddress: text("address"),
  email: text("email"),
  phone: text("phone"),
  website: text("website"),
  portalUrl: text("submission_portal"),
  readingRoom: text("reading_room"),
  submissionMethods: varchar("submission_methods", { length: 16 }).$type<"portal" | "email" | "mail" | "mixed">().default("mixed").notNull(),
  sourceRef: text("source_ref"),
  sourceHash8: text("source_hash8"),
  notes: text("notes"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
...
export const foiaRecordTypes = pgTable("foia_record_types", {
  ...
  typicalKeywords: jsonb("typical_keywords").$type<string[]>(),
...
export const foiaAgencyRecords = pgTable("foia_agency_records", {
  ...
  recordTypeId: integer("record_type_id").notNull(),
  statuteId: integer("statute_id").notNull(),
  confidence: varchar("confidence", { length: 8 }).$type<"high" | "medium" | "low">().default("medium").notNull(),
  notes: text("notes"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
}, (table) => [

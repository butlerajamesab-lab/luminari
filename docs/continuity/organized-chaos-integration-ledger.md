...
The default invocation over repository migrations, migration sources and data now completes. SQL files outside the strict literal-row adapter are retained whole as `held_sql_source_only`, with their source hash and `execution_allowed=false`; no partial extracted rows escape that hold. The canonical reconciliation adapter still rejects unsupported SQL. A default CLI regression includes the migration containing UPDATE statements that previously stopped the build.

Reconciliation previews now serialize declared `text[]` columns as typed PostgreSQL arrays, preserving empty arrays, quoted/backslash text, empty strings and null elements while rejecting nested or non-text values. The schema guard checks PostgreSQL's `ARRAY` / `pg_catalog._text` identity. An isolated PostgreSQL test verifies exact array round trips, mandatory rollback and rejection of a changed integer-array column.
...
- A migration extending the **existing** `sync_luminari_corpus_source_manifest_v2()` selectors to `Batch`. Duplicate classification uses downloaded SHA256 instead of ETag/size alone. Its exact filename/blob hash is registered in the existing repository-only migration ledger; production receipts are unchanged. The migration creates no canonical tables, changes no bucket visibility, and performs no import or download.

...
- New case context uses public.cases and authenticated ownership. The separate legacy case namespace is never joined by coincident numeric IDs. Both context APIs and Sunam dispatch receive caller identity, and other legacy case service tools now verify ownership before accessing private case data.

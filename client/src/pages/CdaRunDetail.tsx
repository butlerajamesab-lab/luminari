...
function T7StatsPanel({ runId }: { runId: number }) {
  ...
  const resolutionMethods = [
    ...
    {
      key: "fallback_ambiguous",
      label: "Fallback Ambiguous",
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="text-xs text-muted-foreground ml-auto">
          {stats.totalComparisons} total comparison
          {stats.totalComparisons !== 1 ? "s" : ""}
        </span>
      </div>
          ...
          {matchTypes.map(({ key, label, color }) => {
            ...
            const pct =
              stats.totalComparisons > 0
                ? Math.round((count / stats.totalComparisons) * 100)
                : 0;
            return (
              <div key={key} className="flex items-center gap-2">
                <Badge
                  variant="outline"
                  className={`text-xs ${color} border-0 min-w-[100px] justify-center`}
                >
                  {label}
                </Badge>
    ...
    <div className="border border-border rounded-lg bg-muted/10 px-4 py-3">
      <p className="text-sm text-foreground">
        ...
        <span className="font-semibold text-primary">
          {data.clauseCount} clause{data.clauseCount !== 1 ? "s" : ""}
        </span>{" "}
        and leaves{" "}
        <span className="font-semibold text-amber-400">
          {data.gapCount} evidence gap{data.gapCount !== 1 ? "s" : ""}
        </span>
...
export default function CdaRunDetail() {
  ...
  return (
    <div className="p-6 space-y-6">
          ...
          {isTerminal && (
            <>
              ...
              <Button
                ...
                onClick={() =>
                  setLocation(
                    `/lumensend?type=appeal&context=cda_denial&contextId=${runId}&state=`,
                  )
                }
              >
        ...
        <div className="border border-border rounded-lg">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            ...
            <TabsContent value="o1" className="p-4 m-0">
              <ForensicMarkdown
                content={artifacts.O1_structured_claim_ledger}
              />
            </TabsContent>
            <TabsContent value="o2" className="p-4 m-0">
              <ForensicMarkdown
                content={artifacts.O2_policy_denial_comparison_matrix}
              />
            </TabsContent>
            <TabsContent value="o3" className="p-4 m-0">
              <ForensicMarkdown
                content={artifacts.O3_evidence_gaps_contradictions}
              />
            </TabsContent>
            <TabsContent value="o4" className="p-4 m-0">
              <ForensicMarkdown
                content={artifacts.O4_advocacy_packet_outline}
              />
            </TabsContent>
          </Tabs>
        </div>
          ...
          {isTerminal ? (
            <>
              ...
              <p className="text-sm text-muted-foreground">
                ...
                {"error" in bundle &&
                  typeof (bundle as Record<string, unknown>).error ===
                    "string" && (
                    <span className="block text-xs text-red-400 mt-1 font-mono">
                      {(bundle as Record<string, unknown>).error as string}
                    </span>
                  )}
              </p>
            </>
            ...
            <>
              ...
              <p className="text-sm text-muted-foreground">
                Pipeline is still running. Artifacts will appear when processing
                completes.
              </p>
            </>
          )}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  LumenSend can draft an appeal letter using this denial
                  analysis as context.
                </p>
              </div>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                onClick={() =>
                  setLocation(
                    `/lumensend?type=complaint&context=cda_denial&contextId=${runId}`,
                  )
                }
              >
              ...
              <Button
                ...
                onClick={() =>
                  setLocation(
                    `/lumensend?type=appeal&context=cda_denial&contextId=${runId}`,
                  )
                }
              >
      ...
      {/* Unmet Criteria */}
      {Array.isArray(run.unmetCriteria) &&
        (run.unmetCriteria as string[]).length > 0 && (

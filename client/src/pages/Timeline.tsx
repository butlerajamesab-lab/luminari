...
import {
  ...
  project_intake_event_to_chronology,
  project_legacy_event_to_chronology,
  sort_chronology_records,
  type intake_chronology_event,
} from "@/lib/chronologyProjection";

function is_intake_chronology_event(value: unknown): value is intake_chronology_event {
  if (!value || typeof value !== "object") return false;
  const event = value as Record<string, unknown>;
  return (
    typeof event.event_id === "string" &&
    (event.date === null || typeof event.date === "string") &&
    typeof event.date_precision === "string" &&
    typeof event.event_text === "string" &&
    typeof event.source_artifact_key === "string" &&
    typeof event.source_span_offset === "number" &&
    typeof event.verification_status === "string"
  );
}
...
export default function Timeline() {
  ...
  const { data: events, isLoading: legacyLoading } = trpc.events.list.useQuery(
  ...
  const { data: intakeProjections, isLoading: intakeLoading } = trpc.analyze.getIntakeSpineLayerProjection.useQuery(
    { caseId: currentCaseId!, layerName: "chronology_reconstruction" },
    { enabled: !!currentCaseId }
  );

  const canonicalRunCount = useMemo(
    () => (intakeProjections ?? []).filter(projection => !!projection.layer_run_id).length,
    [intakeProjections],
  );

  const chronology = useMemo(() => {
    const canonical = (intakeProjections ?? []).flatMap(projection => {
      if (!projection.layer_run_id || !Array.isArray(projection.data)) return [];
      return projection.data
        .filter(is_intake_chronology_event)
        .map(event => project_intake_event_to_chronology(event, {
          intake_session_id: projection.intake_session_id,
    ...
    const canonicalExactKeys = new Set(
      canonical.map(record => `${record.event_date ?? ""}|${record.observed_event.trim()}`),
    );
    const legacy = (events ?? [])
      .map(event => project_legacy_event_to_chronology(event))
      .filter(record => !canonicalExactKeys.has(`${record.event_date ?? ""}|${record.observed_event.trim()}`));

    return sort_chronology_records([...canonical, ...legacy]);
  }, [events, intakeProjections]);
    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Source-aware factual reconstruction from sealed Intake Spine output and preserved case events
        </p>
      </div>
      ...
      <Card className="border-dashed bg-muted/10">
        <CardContent className="p-3 text-xs text-muted-foreground flex items-start gap-2">
          ...
          <p>
            Sealed Universal Intake Spine chronology is rendered directly from its canonical output artifact and receipt.
            Existing case events remain preserved as reported records and are not rewritten or silently promoted.
          </p>
        </CardContent>
      </Card>
      ...
      {canonicalRunCount > 0 && (
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          {canonicalRunCount} sealed chronology projection{canonicalRunCount === 1 ? "" : "s"} attached to this case
        </div>
      )}
        ...
        <Card className="border-dashed">
          <CardContent className="p-8 flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-sm text-muted-foreground">
              {canonicalRunCount > 0
                ? "The sealed Intake Spine chronology contains no date-stated events, and no preserved case events are available."
                : "No chronology events yet. Preserve evidence, then explicitly run the Universal Intake Spine from the case workspace."}
            </p>
        ...
        <div className="relative">
          ...
          <div className="space-y-4">
            {chronology.map(record => {
              const canonical = record.chronology_event_id.startsWith("intake-");
              return (
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex flex-wrap items-center gap-2 mt-2">
                            ...
                            {canonical && <Badge variant="outline" className="text-[10px]">Intake Spine</Badge>}

...
const lifecycle_for_bill = (bill: docket_bill, cache_fresh: boolean): lifecycle_state => {
  ...
  const terminal_evidence = /^\s*(?:chapter(?:ed)?|effective date|enacted|withdrawn|dead|vetoed)\b/.test(action_evidence)
    ...
    || /^\s*postponed indefinitely\b|\b(?:bill|measure|resolution)\s+(?:has\s+)?(?:enacted|failed|withdrawn|vetoed|died|(?:been\s+)?postponed\s+indefinitely|indefinitely\s+postponed)\b|\bfailed\s+(?:final passage|to pass)\b/.test(action_evidence);

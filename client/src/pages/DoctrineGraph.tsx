...
import {
  Network, Loader2, Search, X, Scale, Maximize2, Minimize2,
} from "lucide-react";
...
const NODE_TYPES: Record<string, { color: string; label: string }> = {
  ...
  stage: { color: c.amber, label: "Procedural Stage" },
};
...
const EDGE_TYPES: Record<string, { color: string; label: string }> = {
  ...
  derived_from: { color: c.gold, label: "Derived From" },
  weak_joint: { color: c.red, label: "Weak Joint" },
  links_to: { color: c.teal, label: "Links To" },
};
...
function stringList(value: unknown): string[] {
  ...
  try {
    ...
    // Legacy rows may use comma- or semicolon-separated text.
  }
...
export default function DoctrineGraph() {
  ...
  const graphQuery = trpc.enforcementIntel.getDoctrineGraph.useQuery();
  const graphData = graphQuery.data;
  const doctrineList = graphData?.doctrines;
  ...
  const { nodes, edges } = useMemo(() => {
    ...
    const doctrineIdByName = new Map<string, string>();
    for (const d of graphData.doctrines) {
      doctrineById.set(String(d.id), d.name);
      doctrineIdByName.set(String(d.name).trim().toLowerCase(), String(d.id));
    }

    const endpointId = (type: string, rawId: unknown) => {
      const value = String(rawId ?? "").trim();
      if (type !== "doctrine") return `${type}:${value}`;
      const canonicalId = doctrineById.has(value)
        ? value
        : doctrineIdByName.get(value.toLowerCase());
      return `doctrine:${canonicalId ?? value}`;
    };

    const endpointLabel = (type: string, rawId: unknown) => {
      const value = String(rawId ?? "").trim();
      if (type !== "doctrine") return value;
      const canonicalId = doctrineById.has(value)
        ? value
        : doctrineIdByName.get(value.toLowerCase());
      return canonicalId ? (doctrineById.get(canonicalId) ?? value) : value;
    };
    ...
    for (const e of graphData.edges) {
      const sourceId = endpointId(e.from_type, e.from_id);
      const targetId = endpointId(e.to_type, e.to_id);
      const sourceLabel = endpointLabel(e.from_type, e.from_id);
      const targetLabel = endpointLabel(e.to_type, e.to_id);
  ...
  const filteredNodes = useMemo(() => {
    if (!filterType && !search) return nodes;
    ...
    // A doctrine graph is useful only with its context. Keep directly
    // connected nodes visible when a type or text filter focuses the graph.
    for (const edge of edges) {
      if (matchedIds.has(edge.source)) visibleIds.add(edge.target);
      if (matchedIds.has(edge.target)) visibleIds.add(edge.source);
    }
    return nodes.filter((node) => visibleIds.has(node.id));
  }, [nodes, edges, filterType, search]);
  ...
  if (queryError) {
    return (
      <div style={{ background: c.bg, minHeight: "100vh", padding: "clamp(16px, 3vw, 32px)", fontFamily: fontSans }}>
        <LayerNavBar label="Doctrine Graph" route="/doctrine-graph" />
        <div style={{ maxWidth: 720, margin: "80px auto", background: c.redBg, border: `1px solid ${c.redBorder}`, borderRadius: 12, padding: 24 }}>
          <h1 style={{ color: c.paper, fontFamily: fontSerif, fontSize: 24, margin: "0 0 8px" }}>Doctrine graph unavailable</h1>
          <p style={{ color: c.muted, fontSize: 13, lineHeight: 1.6, margin: "0 0 16px" }}>
            The governed doctrine registry could not be read. No broader civic records have been substituted into this surface.
          </p>
          <button
            type="button"
            onClick={() => { void graphQuery.refetch(); }}
            style={{ background: c.purpleBg, border: `1px solid ${c.purpleBorder}`, borderRadius: 7, color: c.paper, cursor: "pointer", padding: "8px 12px" }}
          >
            Retry doctrine registry
          </button>
        </div>
      </div>
    );
    ...
    <div style={{ background: c.bg, minHeight: "100vh", padding: "clamp(16px, 3vw, 32px)", fontFamily: fontSans }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ color: c.muted, fontSize: 14, margin: 0, maxWidth: 700 }}>
          Every doctrine in the governed registry is included. Statutes, cases, agencies,
          procedural stages, domains, and weak joints appear only when explicitly connected
          to a doctrine. Click any node to explore those relationships.
        </p>
        <nav aria-label="Related legal surfaces" style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
          {[
            ...
            <a
              key={href}
              href={href}
              style={{ color: c.paper, background: c.cardBg, border: `1px solid ${c.cardBorder}`, borderRadius: 7, padding: "6px 9px", textDecoration: "none", fontSize: 11 }}
            >
              {label}
            </a>
          ))}
      ...
      <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
        ...
        <div
          ref={containerRef}
          style={{
            flex: "1 1 620px", minWidth: 0, background: "rgba(0,0,0,0.3)", border: `1px solid ${c.cardBorder}`,
        ...
        {selectedDetails && (
          <div style={{
            flex: "1 1 280px", minWidth: 0, maxWidth: 420, background: c.cardBg, border: `1px solid ${c.cardBorder}`,
            ...
            {selectedDetails.doctrine && (
              <div style={{
                ...
                {stringList(selectedDetails.doctrine.primary_cases).length > 0 && (
                  <div>
                    <span style={{ color: c.muted, fontSize: 11 }}>Primary Cases:</span>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 }}>
                      {stringList(selectedDetails.doctrine.primary_cases).map((cs, i) => (
                        <span key={i} style={{
                          background: c.cardBg, border: `1px solid ${c.cardBorder}`,
                ...
                {stringList(selectedDetails.doctrine.domains).length > 0 && (
                  <div style={{ marginTop: 8 }}>
                    <span style={{ color: c.muted, fontSize: 11 }}>Domains:</span>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 4, marginTop: 4 }}>
                      {stringList(selectedDetails.doctrine.domains).map((d, i) => (
                        <span key={i} style={{
      ...
      {doctrineList && doctrineList.length > 0 && (
        <div style={{ marginTop: 32 }}>
          ...
          <p style={{ color: c.muted, fontSize: 12, margin: "-8px 0 16px" }}>
            {doctrineList.length.toLocaleString()} governed doctrine records. This catalog is registry-driven and has no hard-coded doctrine ceiling.
          </p>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 340px), 1fr))", gap: 12 }}>
            {doctrineList.map(d => (
              <div
                key={d.id}
                onClick={() => setSelectedNode(`doctrine:${d.id}`)}
                style={{
                  background: c.cardBg, border: `1px solid ${c.cardBorder}`,
              ...
              >
                ...
                <p style={{ color: c.muted, fontSize: 12, lineHeight: 1.5, margin: "0 0 8px 0" }}>
                  {String(d.description ?? "").length > 150
                    ? String(d.description ?? "").slice(0, 147) + "..."
                    : String(d.description ?? "")}
                </p>
                <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
                  {stringList(d.domains).map((dom, i) => (
                    <span key={i} style={{
                      background: c.goldBg, border: `1px solid ${c.goldBorder}`,

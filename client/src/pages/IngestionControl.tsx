...
function extraction_has_failure(result: extract_diagnostic | null) {
  ...
  return result.success === false || result.command_failed === true || failures > 0;
}
  ...
  const extract_docx_queue_row = async (id: number, dry_run: boolean) => {
    ...
    try {
      ...
      if (extraction_has_failure(result)) {
        throw new Error(result.message ?? result.error ?? "extract_docx_command_reported_failure");
      }
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Link href="/sovereign-control"><Button variant="outline" size="sm" className="gap-1.5"><ArrowLeft className="h-4 w-4" />Sovereign Control</Button></Link>
          <div><h1 className="text-2xl font-bold text-foreground flex items-center gap-2"><Database className="h-5 w-5 text-cyan-400" />ingestion_control</h1><p className="text-sm text-muted-foreground">Server-backed queue viewer for bucket and corpus staging. No canonical promotion.</p></div>
        </div>
      ...
      <div className="grid grid-cols-4 gap-3"><Card className="bg-card/50"><CardContent className="p-3"><div className="text-xs text-muted-foreground">visible_rows</div><div className="text-xl font-bold">{summary.total_rows}</div></CardContent></Card><Card className="bg-card/50"><CardContent className="p-3"><div className="text-xs text-muted-foreground">blocked_rows</div><div className="text-xl font-bold text-amber-400">{summary.blocked_rows}</div></CardContent></Card><Card className="bg-card/50"><CardContent className="p-3"><div className="text-xs text-muted-foreground">review_required</div><div className="text-xl font-bold text-red-400">{summary.review_required_rows}</div></CardContent></Card><Card className="bg-card/50"><CardContent className="p-3"><div className="text-xs text-muted-foreground">strict_authority</div><div className="text-xl font-bold text-purple-400">{summary.strict_authority_rows}</div></CardContent></Card></div>
      ...
      {extract_diagnostic && (
        <Card className={diagnostic_failed ? "border-red-500/30 bg-red-950/10" : "border-green-500/30 bg-green-950/10"}>
          <CardContent className="p-3 space-y-2 text-xs">
            <div className="font-semibold">extract_diagnostic: {extract_diagnostic.error ?? (diagnostic_failed ? "extract_docx_command_reported_failure" : "success")}</div>
            <div>state_changed: {String(extract_diagnostic.state_changed ?? false)} · command_failed: {String(extract_diagnostic.command_failed ?? false)} · command_extracted: {String(extract_diagnostic.command_extracted ?? false)} · dry_run: {String(extract_diagnostic.dry_run ?? false)} · runtime_ms: {extract_diagnostic.runtime_ms ?? "—"}</div>
            {extract_diagnostic.message && <div>{extract_diagnostic.message}</div>}
            {extract_diagnostic.command_summary && <pre className="text-[10px] whitespace-pre-wrap bg-muted/30 rounded p-2 max-h-40 overflow-auto">command_summary:{"\n"}{JSON.stringify(extract_diagnostic.command_summary, null, 2)}</pre>}

...
const status_label = (status: unknown, last_action: unknown): string => {
  ...
  if ([5, 6].includes(Number(status)) || /^\s*(?:chapter(?:ed)?|effective date|enacted|withdrawn|dead|vetoed|postponed indefinitely)\b/.test(evidence) || /signed by governor|governor signed|became law/.test(evidence) || /\b(?:bill|measure|resolution)\s+(?:has\s+)?(?:enacted|failed(?:\s+(?:final passage|to pass))?|withdrawn|vetoed|died|(?:been\s+)?postponed\s+indefinitely|indefinitely\s+postponed)\b/.test(evidence)) return "Completed";

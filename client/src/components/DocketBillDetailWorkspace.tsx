...
const status_label = (status: unknown, last_action: unknown): string => {
  ...
  if ([5, 6].includes(Number(status)) || /^\s*(?:chapter(?:ed)?|effective date|enacted|withdrawn|dead|vetoed)\b/.test(evidence) || /signed by governor|became law|postponed indefinitely/.test(evidence) || /\b(?:bill|measure|resolution)\s+(?:has\s+)?(?:enacted|failed|withdrawn|vetoed|died)\b|\bfailed\s+(?:final passage|to pass)\b/.test(evidence)) return "Completed";

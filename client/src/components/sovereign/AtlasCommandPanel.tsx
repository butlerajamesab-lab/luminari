...
import AtlasResultList from "./AtlasResultList";
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {catalogPreview.length === 0 ? (
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Load the Atlas catalog to preview available streams.</div>
          ) : (
            catalogPreview.map((stream) => (
              <div key={stream.stream_id} className="grid gap-2 rounded-lg border p-3 text-sm md:grid-cols-5">
                <div><div className="text-xs text-muted-foreground">stream_id</div><div className="font-mono text-xs text-foreground">{stream.stream_id}</div></div>
                <div><div className="text-xs text-muted-foreground">stream_name</div><div className="text-foreground">{stream.stream_name ?? "—"}</div></div>
                <div><div className="text-xs text-muted-foreground">jurisdiction</div><div className="text-foreground">{String(stream.jurisdiction ?? "—")}</div></div>
                <div><div className="text-xs text-muted-foreground">domain</div><div className="text-foreground">{String(stream.domain ?? "—")}</div></div>
                <div><div className="text-xs text-muted-foreground">source</div><div className="break-words text-xs text-foreground">{stream.source_url ?? stream.api_url ?? "—"}</div></div>
              </div>
            ))
          )}
          {catalogStreams.length > catalogPreview.length && <div className="text-xs text-muted-foreground">Showing {catalogPreview.length} of {catalogStreams.length} streams.</div>}
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {!lastResult ? (
            <div className="rounded-lg border border-dashed p-6 text-center text-sm text-muted-foreground">Run Populate Atlas Streams or Drain Bridge to view results.</div>
          ) : (
            <>
              <div className="grid gap-3 md:grid-cols-4">
                <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">created</div><div className="text-xl font-bold text-green-400">{listCount(lastResult.created_stream_ids)}</div></div>
                <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">skipped</div><div className="text-xl font-bold text-amber-400">{listCount(lastResult.skipped_stream_ids)}</div></div>
                <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">failed</div><div className="text-xl font-bold text-red-400">{listCount(lastResult.failed_stream_ids)}</div></div>
                <div className="rounded-lg border p-3"><div className="text-xs text-muted-foreground">total_selected</div><div className="text-xl font-bold text-foreground">{lastResult.total_selected ?? "—"}</div></div>
              </div>
              <div className="grid gap-3 md:grid-cols-3">
                <AtlasResultList title="created_stream_ids" values={lastResult.created_stream_ids} />
                <AtlasResultList title="skipped_stream_ids" values={lastResult.skipped_stream_ids} />
                <AtlasResultList title="failed_stream_ids" values={lastResult.failed_stream_ids} />
              </div>
              {lastResult.errors && <pre className="max-h-48 overflow-auto rounded-lg bg-muted p-3 text-xs text-muted-foreground">{JSON.stringify(lastResult.errors, null, 2)}</pre>}
            </>
          )}
        </CardContent>
      </Card>

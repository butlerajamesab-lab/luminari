 ...
 * makes truncation a property of the same snapshot as the exported rows, so a
 * concurrent insert cannot make a signed bundle silently incomplete.
 */
export async function export_spine_table_data_consistent(
  ...
  const result = await query_with_diagnostics<Record<string, unknown>>(
    `select * from public.${table} limit ${probe_limit}`,
    [],
    {

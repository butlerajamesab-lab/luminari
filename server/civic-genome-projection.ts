...
export const infer_state_position = (bill: legiscan_master_bill): string => {
  ...
  if (/^\s*(?:failed|withdrawn|dead|vetoed|postponed indefinitely)\b|\b(?:bill|measure|resolution)\s+(?:has\s+)?(?:failed|withdrawn|vetoed|died|(?:been\s+)?postponed\s+indefinitely|indefinitely\s+postponed)\b/.test(last_action))
    return "failed";

...
describe("canonical spine semantic civic graph v2 contract", () => {
  const relationshipMigration = read("../supabase/migrations/20260816161657_lighthouse_explicit_civic_relationship_graph_v2.sql");
  const stateOptimization = read("../supabase/migrations/20260816162036_optimize_lighthouse_canonical_state_v2.sql");
  ...
  it("keeps structural graph v1 and adds explicit semantic relationships as v2", () => {
    expect(relationshipMigration).toContain("public.v_lighthouse_graph_edges_v1");
    expect(relationshipMigration).toContain("public.v_lighthouse_graph_relationship_edges_v1");
    expect(relationshipMigration).toContain("public.v_lighthouse_graph_edges_v2");
    expect(relationshipMigration).toContain("'source_declared_exact'::text as evidence_state");
  });
  ...
  it("resolves only source-declared exact relationships and preserves unresolved declarations", () => {
    ...
    expect(relationshipMigration).toContain("public.v_lighthouse_graph_unresolved_relationships_v1");
  ...
  it("cuts canonicalCore reads over to v2 and exposes bounded edge surfaces", () => {
    ...
    expect(service).toContain("public.v_lighthouse_graph_relationship_edges_v1");
    expect(service).toContain("public.v_lighthouse_graph_edges_v2");
    expect(service).toContain("public.v_lighthouse_graph_unresolved_relationships_v1");
  ...
  it("makes Mission Control distinguish structural, semantic, and unresolved graph state", () => {
    ...
    expect(panel).toContain("Unresolved Relationship Declarations");

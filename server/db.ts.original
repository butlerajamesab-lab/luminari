...
// 1. Extract credentials and connection info from DATABASE_URL
const host = dbUrl.hostname;
const port = dbUrl.port ? parseInt(dbUrl.port, 10) : 4000;
const user = decodeURIComponent(dbUrl.username);
const password = decodeURIComponent(dbUrl.password);

// 2. Extract database name without hardcoding (removes leading slash)
const database = dbUrl.pathname.replace(/^\//, '');

// Parse SSL settings from query params if needed, or enforce for TiDB Cloud
const sslConfig = {
  rejectUnauthorized: true
};

// 4 & 5. Pool configuration using parsed credentials and mysql2
const poolConfig = {
  host,
  port,
  user,
  password,
  database,
  ssl: sslConfig, // 6. Enables SSL for TiDB Cloud
...
export async function verifyConnection() {
  try {
    ...
    console.log(`Successfully connected to database: ${database} at ${host}`);
    connection.release();
  } catch (error) {
    console.error(`Failed to connect to the database ${database}:`, error);
    process.exit(1);
  }
}

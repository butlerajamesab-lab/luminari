...
export const conduitRouter = router({
  ...
  metadataHealth: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [fieldStats] = await ctx.db.execute(sql`
    ...
    const [eventCounts] = await ctx.db.execute(sql`
    ...
    const [totalTables] = await ctx.db.execute(sql`SELECT COUNT(*) as cnt FROM table_registry`);
    const [totalFields] = await ctx.db.execute(sql`SELECT COUNT(*) as cnt FROM field_dictionary`);

    return {
      tables: {
        total: (totalTables as any[])[0]?.cnt || 0,
        byCategory: tableRows as any[],
      },
      fields: {
        total: (totalFields as any[])[0]?.cnt || 0,
        stats: (fieldStats as any[])[0] || { totalFields: 0, pkFields: 0, indexedFields: 0 },
      },
      drift: {
  ...
  pipelineIntegrity: protectedProcedure.query(async ({ ctx }) => {
    ...
    const [recentRuns] = await ctx.db.execute(sql`
    ...
    const [formatDist] = await ctx.db.execute(sql`
      SELECT 
        COUNT(*) as total,
    ...
    return {
      ...
      backboneCounts,
      formatDistribution: (formatDist as any[])[0] || { total: 0, no_refs: 0, deterministic: 0, legacy: 0 },
    };
  }),
  ...
  exportReadiness: protectedProcedure.query(async ({ ctx }) => {
    ...
    // Alpha Lake exports
    const [exports] = await ctx.db.execute(sql`
    ...
    for (const snap of (snapshots as any[]).slice(0, 5)) {
    ...
    const [exportEvents] = await ctx.db.execute(sql`
    ...
    return {
      ...
      readySnapshots,
      totalAlphaExports: (exportEvents as any[])[0]?.cnt || 0,
    };
  }),

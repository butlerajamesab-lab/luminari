...
export const knowledgeIngestionRouter = router({
  ...
  browseAgencies: protectedProcedure
    ...
    .query(async ({ input }) => {
      ...
      const arNames = new Set((arRows as any[]).map((r: any) => r.agencyName?.toLowerCase()));
      const aamFiltered = (aamRows as any[]).filter((r: any) => !arNames.has(r.agencyName?.toLowerCase()));

      // Normalize agencies_registry rows to match UI shape
      const normalizedAR = (arRows as any[]).map((r: any) => {

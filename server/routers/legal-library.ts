...
export const legalLibraryRouter = router({
  ...
  createStatute: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const id = await createStatute({ ...input, domains: castDomains(input.domains), addedBy: ctx.user.name ?? ctx.user.open_id ?? "" } as any);
      return { id };
    }),
  ...
  createEnforcement: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      const id = await createEnforcementRecord({ ...input, domains: castDomains(input.domains), addedBy: ctx.user.name ?? ctx.user.open_id ?? "" } as any);
      return { id };
    }),

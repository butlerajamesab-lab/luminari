/**
 * Public Admin Maintenance Router
 * 
 * One-time setup endpoints for database maintenance.
 * These are public because they're one-time operations that must complete during setup.
 */
...
export const publicAdminMaintenanceRouter = router({
  /**
   ...
   * Public access because it's a setup operation.
   */
  backfillConfidenceScores: publicProcedure
    .query(async () => {
      try {
        ...
        console.log('[Backfill] Confidence scores updated permanently');
        console.log(`[Backfill] Total: ${s.total}, High: ${s.high}, Medium-High: ${s.medium_high}, Medium: ${s.medium}, Low: ${s.low}, Avg: ${s.avg_score}`);
   ...
   */
  checkConfidenceStats: publicProcedure
    .query(async () => {
      const stats = await db.execute(sql.raw(`
        SELECT 
          COUNT(*) as total,

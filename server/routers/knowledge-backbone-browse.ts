...
function payload(value: unknown): PagedPayload {
  if (!value || typeof value !== "object") return { rows: [], total: 0 };
  const candidate = value as { rows?: unknown; total?: unknown };
  return {
    rows: Array.isArray(candidate.rows) ? candidate.rows as Record<string, unknown>[] : [],
...
export const knowledgeBackboneBrowseRouter = router({
  ...
  browseAgencies: protectedProcedure.input(pageInput.omit({ domain: true })).query(async ({ input }) => {
    const pool = getPool();
    const { rows } = await pool.query(
      `with filtered as (
  ...
  browseCourts: protectedProcedure.input(z.object({
    search: z.string().trim().max(160).optional(),
    jurisdiction: z.string().trim().max(120).optional(),
  ...
  browseAdvocacyTargets: protectedProcedure.input(z.object({
    search: z.string().trim().max(160).optional(),
  ...
  browseSettlementFormulas: protectedProcedure.input(z.object({
    search: z.string().trim().max(160).optional(),
  ...
  getJurisdictions: protectedProcedure.query(async () => {
    ...
    const { rows } = await pool.query(`
      ...
      where jurisdiction is not null and btrim(jurisdiction) <> '' and lower(jurisdiction) <> 'state'
    ...
    return Array.from(unique).sort((a, b) => a === "Federal" ? -1 : b === "Federal" ? 1 : a.localeCompare(b));
  }),
  ...
  getDomains: protectedProcedure.query(async () => {
    ...
    for (const row of rows as Array<{ domains: unknown }>) {
      ...
      if (Array.isArray(value)) {
        for (const domain of value) if (typeof domain === "string" && domain.trim()) domainSet.add(domain.trim());
      } else if (typeof value === "string" && value.trim()) {
        try {
          const parsed = JSON.parse(value);
          if (Array.isArray(parsed)) for (const domain of parsed) if (typeof domain === "string" && domain.trim()) domainSet.add(domain.trim());
          else domainSet.add(value.trim());
        } catch {

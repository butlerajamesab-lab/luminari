...
const prismPresentation = readFileSync(
  join(root, "shared", "civic-genome-prism-presentation.ts"),
  "utf8",
);
...
const dock = readFileSync(
  join(
    ...
    "components",
    "civic-genome",
    "CivicGenomeExportDock.tsx",
  ),
  "utf8",
);
const prismProof = readFileSync(
  join(root, "client", "src", "components", "civic-genome", "PrismProof.tsx"),
  "utf8",
);
...
describe("Civic Genome export contract", () => {
  it("preserves raw JSON attachments as technical companion data", () => {
    expect(route).toContain(
      'civic_genome_export_router.get("/bill/:source_bill_id"',
    );
  ...
  it("adds summary and detailed human-readable reports backed by the exact Rosetta source snapshot", () => {
    expect(route).toContain('"/bill/:source_bill_id/summary"');
    expect(route).toContain('"/bill/:source_bill_id/detailed"');
    expect(route).toContain("text/html; charset=utf-8");
    expect(route).toContain(
      "render_civic_genome_human_report(human_payload, mode)",
    );
  ...
  it("classifies human Prism results without inventing a final-bill comparison", () => {
    expect(prismPresentation).toContain(

...
/**
 * Optional bounded production acceptance proof.
 *
 * Normal deployments leave both variables empty. When configured, startup
 * reads one exact family twice at one exact as-of time through a repeatable-read
 * read-only transaction. It logs hashes and counts only; it performs no write
 * and does not emit the source payload.
 */

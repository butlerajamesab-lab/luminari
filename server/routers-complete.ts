...
import { enqueueDocument, runCrossDocumentCorrelation, reanalyzeDocument, reanalyzeAllDocuments, getQueueStatus } from "./analysis-pipeline";
...
const activationRouter = router({
  ...
  getAll: publicProcedure.query(async ({ ctx }) => {
    try {
      const all = await ctx.db.select().from(activationOutputs);
  ...
  start: publicProcedure.input(z.object({ clusterId: z.string() })).mutation(async ({ ctx, input }) => {
    const now = Date.now();
    await ctx.db.execute(sql`UPDATE activation_outputs SET status = 'in_progress', updated_at = ${now} WHERE cluster_id = ${input.clusterId}`);
    return { success: true };
  }),
  complete: publicProcedure.input(z.object({ clusterId: z.string() })).mutation(async ({ ctx, input }) => {
    const now = Date.now();
    await ctx.db.execute(sql`UPDATE activation_outputs SET status = 'completed', updated_at = ${now} WHERE cluster_id = ${input.clusterId}`);
    return { success: true };
  }),
});
...
const documentsRouter = router({
  list: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      await dbHelpers.verifyCaseOwnership(input.caseId, ctx.user.id);
      return dbHelpers.listDocuments(input.caseId);
    }),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      for (const doc of bundle.documents) {
        try {
          ...
          if (!response.ok) {
            console.error(`[TestLoader] Failed to fetch ${doc.filename}: ${response.status}`);
            continue;
          }
          ...
          console.error(`[TestLoader] Error processing ${doc.filename}:`, err);
        }
      }

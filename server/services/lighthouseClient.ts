 ...
 *   - NEVER reads from substrate tables directly
...
function lighthouseUrl(): string {
  ...
  if (!url) throw new Error("[lighthouseClient] SUPABASE_URL is not configured");
  return url.replace(/\/$/, "");
}

function lighthouseKey(): string {
  const key = ENV.lighthouseSupabaseServiceRoleKey;
  if (!key) throw new Error("[lighthouseClient] SUPABASE_SERVICE_ROLE_KEY is not configured");
  return key;
}

...
function queue_guarded_label(label: string): boolean {
  return [
...
function record_timeout(label: string): void {
...
export async function query_with_diagnostics<T = any>(
  ...
  try {
    ...
    if (classify_db_error(error) === "query_timeout") {
      record_timeout(label);
      throw new DbTimeoutDiagnosticError(
        "query_timeout",

...
describe("Civic Genome Atlas handoff",()=>{
 ...
 it("uses the Atlas-specific current-cursor producer rather than a stale configured timestamp",()=>{const startup=readFileSync(new URL("./civic-genome-atlas-handoff-startup.ts",import.meta.url),"utf8");expect(startup).toContain("produce_current_civic_genome_atlas_family_snapshot_v1");expect(startup).toContain("as_of_floor: configuration.as_of");expect(startup).toContain("effective_as_of: snapshot.as_of");expect(startup).not.toContain("produce_civic_genome_atlas_family_snapshot_v1({");expect(startup).not.toContain("build_civic_genome_family_snapshot_v1");});
 it("keeps Atlas bill-version enrichment after current-cursor resolution",()=>{const producer=readFileSync(new URL("./civic-genome-atlas-snapshot-producer.ts",import.meta.url),"utf8");expect(producer).toContain("produce_current_civic_genome_family_snapshot_v1(options)");expect(producer).toContain("enrich_civic_genome_atlas_family_snapshot_v1(base, options.family_id, options)");});
 it("Atlas-specific producer carries an ordered source-bound bill-version manifest",()=>{const producer=readFileSync(new URL("./civic-genome-atlas-snapshot-producer.ts",import.meta.url),"utf8");expect(producer).toContain("civic_genome_external_family_snapshot.1.1.0");expect(producer).toContain("bill_versions");expect(producer).toContain("civic_genome_bill_version_manifest");expect(producer).toContain("stage_rank_then_provider_sequence_then_source_document_key");expect(producer).toContain("source_native_preserved");});
 it("recomputes enriched bill component hash without including the stale source component hash",()=>{const producer=readFileSync(new URL("./civic-genome-atlas-snapshot-producer.ts",import.meta.url),"utf8");expect(producer).toContain("component_hash: _stale_component_hash");expect(producer).toContain("...component_without_hash");});
});

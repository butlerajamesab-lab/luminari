...
describe("Civic Genome Atlas handoff",()=>{
 ...
 it("uses the Atlas-specific governed producer rather than a pure dataset builder",()=>{const startup=readFileSync(new URL("./civic-genome-atlas-handoff-startup.ts",import.meta.url),"utf8");expect(startup).toContain("produce_civic_genome_atlas_family_snapshot_v1");expect(startup).not.toContain("build_civic_genome_family_snapshot_v1");});

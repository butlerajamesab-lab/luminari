...
describe("Universal Intake Spine generic layer execution receipt contract", () => {
  it("preserves strict source-document receipts while admitting generic layer execution receipts", () => {
    expect(genericReceiptMigration).toContain("receipt_type' = 'layer_execution");
    expect(genericReceiptMigration).toContain("receipt_type' in ('evidence_preservation', 'document_replacement')");
    expect(genericReceiptMigration).toContain("luminari.intake.layer-execution.v1");
  ...
  it("makes PostgreSQL canonical-json.v2 authoritative for persisted execution and output hashes", () => {
    expect(crossRuntimeMigration).toContain("luminari_canonical_json_v2(p_execution_envelope)");
    expect(crossRuntimeMigration).toContain("luminari_canonical_json_v2(coalesce(p_output_data, 'null'::jsonb))");
    expect(crossRuntimeMigration).toContain("cross-runtime execution hash mismatch");
    expect(crossRuntimeMigration).toContain("cross-runtime output hash mismatch");
    expect(crossRuntimeMigration).toContain("luminari.intake.cross-runtime-canonical-proof.v1");
  });

  it("forces runtime callers through v3 rather than bypassing cross-runtime verification", () => {
    expect(crossRuntimeMigration).toContain("revoke all on function public.register_intake_layer_execution_v2");
    expect(crossRuntimeMigration).toContain("from service_role");
    expect(crossRuntimeMigration).toContain("grant execute on function public.register_intake_layer_execution_v3");
    expect(verification).toContain("service_role must not bypass the cross-runtime v3 verifier through v2");
  });

...
export async function execute_intake_spine_session(
  ...
  try {
    ...
    await persist_execution_layer({
      ...
      canonical_input: {
        stabilization_input: canonical_stabilization_input(stabilization_input),
        as_of,
      },
      input_refs: [
        {
    ...
    for (const artifact of source_artifacts) {
      const expected_size = normalize_byte_size(
        artifact.byte_size,
        artifact.artifact_id,
      );
      if (!artifact.sha256 || !SHA256_RE.test(artifact.sha256)) {
        throw new Error(
          `intake_spine_orchestrator_invalid_source_sha256:${artifact.artifact_id}`,
        );
      }
      ...
      const l3 = processLayer3(
        { record: l2.data, actual_bytes: source.bytes },
        as_of,
      );
      ...
      if (parsed.raw_bytes_sha256 !== source.verified_sha256) {
        throw new Error(
          `intake_spine_orchestrator_parser_raw_hash_mismatch:${artifact.artifact_id}`,
        );
      }
      if (parsed.parser_rule_manifest_hash !== PARSER_RULE_MANIFEST_HASH) {
        throw new Error(
          `intake_spine_orchestrator_parser_manifest_mismatch:${artifact.artifact_id}`,
        );
      }
    ...
    const parser_input_manifest = parsed_artifacts
      .map((parsed) => ({
        ...
        parsed_output_hash: computeHash({
    ...
    const l13Persisted = await persist_execution_layer({
      ...
      input_refs: [
        dependency_ref(l4Persisted),
        dependency_ref(l6Persisted),
        dependency_ref(l12Persisted),
      ],
      receipts,
      dependencies,
...
async function persist_layer<T>(
  ...
  if (!SHA256_RE.test(input.rule_manifest_hash)) {
    throw new Error(
      `intake_spine_orchestrator_rule_manifest_hash_invalid:${input.result.layer_name}`,
    );
  }
  ...
  if (output_hash !== input.result.output_hash) {
    throw new Error(
      `intake_spine_orchestrator_engine_output_hash_mismatch:${input.result.layer_name}`,
    );
  }
  ...
  if (input.dependency_key)
    input.dependencies.set(input.dependency_key, dependency);
  input.receipts.push({
    layer_name: input.result.layer_name,
    layer_version: input.result.layer_version,
    rule_version: input.result.rule_version,
...
function canonical_stabilization_input(

...
export const lighthouseLineageRouter = router({
  ...
  /** Lineage coverage stats — how many signals have complete chain vs partial */
  coverageStats: protectedProcedure.query(async () => {
    const lineage = await getSignalLineage({ limit: 500 });
    let fullChain = 0;
    let patternLinked = 0;
    let trendLinked = 0;
    let strategyLinked = 0;
    let promoted = 0;
    ...
    return {
      ...
      fullChain,
      promotionRate:
        lineage.length > 0 ? Math.round((promoted / lineage.length) * 100) : 0,
      fullChainRate:
        promoted > 0 ? Math.round((fullChain / promoted) * 100) : 0,
    };
  }),
});

/**
 * Stream Registration - Clean & Register
 * Delete orphaned rows, then register 12 public government data streams
 */
...
export const streamRegisterCleanRouter = router({
  /**
   * Clean orphaned rows and register all 12 streams
   */
  cleanAndRegister: publicProcedure
    .query(async () => {
      try {
        // Step 1: Delete orphaned rows (empty stream_id_dsr)
        console.log('[Stream Register] Cleaning orphaned rows...');
        await db.execute(sql`
          DELETE FROM data_stream_registry 
          WHERE stream_id_dsr = '' OR stream_id_dsr IS NULL
        `);
        console.log('[Stream Register] ✓ Orphaned rows deleted');

        // Step 2: Register all 12 streams
        ...
        for (const stream of STREAMS) {
          try {
            ...
            console.error(`[Stream Register] ✗ ${stream.id}:`, (error as any).message);
          }
        }

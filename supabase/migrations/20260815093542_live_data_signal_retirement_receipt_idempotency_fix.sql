...
begin
  ...
  -- A previously issued Lighthouse record ID is the strongest downstream identity.
  -- It is still bound to the exact Atlas candidate ID and hash, so a stale receipt
  -- cannot retire a newer projection. Historical semantic-key implementations do
  -- not have to be recomputed to accept an idempotent retirement.
...
comment on function public.retire_live_data_signal_transport_receipt_v1(jsonb) is
  'Receives governed Atlas negative-currentness receipts. A prior Lighthouse record ID plus exact Atlas candidate ID/hash is authoritative for historical projection identity; already-noncurrent exact records return idempotent receipts, and qualified column references avoid PL/pgSQL OUT-parameter ambiguity.';

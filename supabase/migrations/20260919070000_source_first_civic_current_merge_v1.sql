-- Source-first current-state merge.
--
-- A newer partial corpus run must not erase valid source-derived objects simply
-- because it failed to reproduce them. Include every non-superseded completed
-- fresh/enrichment generation and select the newest/highest-priority row per
-- exact source candidate hash. Historical rows remain intact; this changes only

...
-- Preserve the prior safe grant/RPC/search_path hardening migration. Optional
-- relations remain capability-gated so clean replay matches the live sequence.
...
begin
  ...
  loop
    if exists (
      ...
      execute format('alter view public.%I set (security_invoker = false)', relation_name);
    end if;
  end loop;
end
$views$;

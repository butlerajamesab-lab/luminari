...
-- Keep the linear predecessor join while removing static references to a
-- session-temporary relation. The normalized source CTE is evaluated once per
-- reconciliation stage and remains range-scoped for bootstrap calls.
...
comment on function public.sync_civic_genome_lifecycle_history_v3(integer) is
  'Lint-safe linear correction-aware reconciliation for one source bill or a transaction-scoped inclusive source-bill range.';

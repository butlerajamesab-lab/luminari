-- Append correction partners for subsidiary dispositions with intervening
-- provider phrasing, such as "Amendment was withdrawn". The bounded matcher
-- cannot cross into unrelated narrative text.
with invalid_subsidiary_terminal_event as (
  ...
  where event.event_type in ('failed', 'vetoed')
    ...
    and coalesce(event.event_payload_json ->> 'event_summary', '')
      ~* 'Latest action:[[:space:]]*(amendment|motion)\y.{0,80}\y(failed|withdrawn|dead|vetoed|postponed indefinitely|indefinitely postponed)\y'
    and not exists (
      select 1

...
create or replace function public.mark_corpus_import_queue_extract_success(
  p_row_id bigint,
  p_worker_id text,
  p_raw_text text,
...
as $$
  ...
  set
    ...
    raw_text = p_raw_text,
    record_count_estimate = greatest(coalesce(record_count_estimate, 0), array_length(regexp_split_to_array(coalesce(p_raw_text, ''), E'\n+'), 1)),
    payload = coalesce(payload, '{}'::jsonb) || jsonb_build_object('worker_extract_result', p_operation_result_json),
...
create or replace function public.mark_corpus_import_queue_normalize_success(
  p_row_id bigint,
  p_worker_id text,
...
as $$
  ...
  set
    ...
    operation_result_json = coalesce(operation_result_json, '{}'::jsonb) || p_operation_result_json,
    payload = coalesce(payload, '{}'::jsonb) || jsonb_build_object('worker_normalize_result', p_operation_result_json),
    worker_state = 'completed_step',
    leased_by = null,
    lease_expires_at = null,
    last_error_code = null,
...
create or replace function public.mark_corpus_import_queue_route_dry_run_success(
  p_row_id bigint,
  p_worker_id text,
  p_route_plan jsonb
)
...
as $$
  update public.corpus_import_queue
  set
    payload = coalesce(payload, '{}'::jsonb) || jsonb_build_object('last_route_dry_run', jsonb_build_object('planned_at', now(), 'plan', p_route_plan)),
    operation_result_json = coalesce(operation_result_json, '{}'::jsonb) || jsonb_build_object('dry_run_plan', p_route_plan),
...
as $$
  ...
  set
    ...
    payload = coalesce(payload, '{}'::jsonb) || jsonb_build_object('last_worker_failure', jsonb_build_object('error_code', p_error_code, 'error_message', left(p_error_message, 4000), 'retryable', p_retryable, 'recorded_at', now())),
    operation_result_json = coalesce(operation_result_json, '{}'::jsonb) || p_operation_result_json,

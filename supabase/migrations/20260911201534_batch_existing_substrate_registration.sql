-- Extend the existing manifest. No new canonical tables, source deletes, imports, or public bucket policy.
...
begin
  ...
  with ranked as (
    select artifact_key,
      first_value(artifact_key) over(partition by content_sha256,byte_size order by artifact_key) canonical_artifact,
      row_number() over(partition by content_sha256,byte_size order by artifact_key) duplicate_rank
...
-- Registration is invoked by the scoped admin queue operation. It does not feed
-- Batch into the legacy queue's automatic SQL execution or promotion actions.
-- No downloads, source SQL execution, or canonical promotion during migration.

...
create or replace function public.rosetta_open_structural_repair_count(
...
revoke all on function public.rosetta_open_structural_repair_count(integer)
  from public

grant execute on function public.rosetta_open_structural_repair_count(integer)
  to anon, authenticated, service_role

do $migration$
begin
  if to_regclass('public.v_civic_genome_law_view_v1_internal') is null then
    if to_regclass('public.v_civic_genome_law_view_v1') is null then
      raise exception 'rosetta_publication_gate_source_view_missing';
    end if;
    execute 'alter view public.v_civic_genome_law_view_v1 rename to v_civic_genome_law_view_v1_internal';
  end if;
end;
$migration$
...
select
  ...
  law.completed_at,
  case
    when public.rosetta_open_structural_repair_count(law.extraction_run_id) > 0
      then '[]'::jsonb
    else law.objects
  end as objects,
  law.coverage,
  ...
  proof_rows as (
    ...
    where cr.run_status in ('completed', 'validated')
      ...
      and not exists (
        select 1
        from public.rosetta_structural_repair_queue repair
        where repair.extraction_run_id = cr.id
          and repair.repair_state in ('open', 'in_review')
      )
    order by cr.id desc
...
comment on view public.v_civic_genome_law_view_v1 is
  'Fail-closed Rosetta/Civic Genome publication view. Runs with open or in-review structural repairs remain inspectable in canonical storage but publish no law objects and cannot report complete provenance.'

comment on function public.rosetta_multi_law_proof_v1(integer, integer) is
  'Bounded Rosetta proof read model. Excludes every extraction run with an open or in-review structural repair.'

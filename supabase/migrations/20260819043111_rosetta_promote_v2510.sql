...
begin
  ...
  select exists(
    ...
    join public.validation_result independent on independent.extraction_run_id=run.id
      and independent.test_name='independent_structure_v2510'
    ...
    where run.source_document_id=p_source_document_id
      ...
      and run.engine_version='rosetta-v3-deterministic-sql-2.5.10'
      and run.rule_set_version='rosetta-five-layer-structural-correctness-2.5.10'
      and run.rule_manifest_hash='46f75737deaf11ee4cc352087ca67d2f7e57545fa9835ac281e4dab32b9cc674'
  ...
  v_receipt:=public.run_rosetta_v3_extraction_v2510_candidate(
  ...
  return v_receipt||jsonb_build_object(
    ...
    'canonical_producer_contract','rosetta-current-producer-v2510',
    'canonical_replay_contract',case when v_was_finalized
      then 'rosetta-finalized-generation-immutable-replay-v2510'
      else 'rosetta-final-generation-produced-v2510'
    end
  );
end;
$$
...
set contract='rosetta-current-generation-v1',
    ...
    rule_manifest_hash='46f75737deaf11ee4cc352087ca67d2f7e57545fa9835ac281e4dab32b9cc674',
    validation_test_name='independent_structure_v2510',
...
comment on function public.run_rosetta_v3_extraction(integer,text,text,text,text,text,text,text,date,text,jsonb) is
  'Canonical Rosetta producer for registry-selected generation 2.5.10. Exact finalized replay is immutable and the canonical producer applies amendment-aware validation only after non-operative amendment projection.'

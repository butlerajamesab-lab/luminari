-- Drift is publishable only when both the independently resolved base and
-- the latest version have completed verified extraction. This preserves
-- removed classes while preventing activation lag from masquerading as removal.
...
COMMENT ON VIEW public.docket_bill_drift_delta IS
  'Per-bill trait-class structural delta over base/latest class union. Base is resolved independently. Both coverage flags must be true before consumers claim drift.';

...
begin
  ...
  loop
    ...
    v_chamber := null;
    v_version_type := public.docket_legislative_version_type(v_document_family, v_provider_type, v_chamber);
    v_stage_rank := public.docket_legislative_stage_rank(v_version_type);
    v_source_url := coalesce(nullif(v_document ->> 'state_link', ''), nullif(v_document ->> 'url', ''));
    ...
    v_provider_date := case
      when coalesce(v_document ->> 'date', '') ~ '^\d{4}-\d{2}-\d{2}$'
       and v_document ->> 'date' <> '0000-00-00' then (v_document ->> 'date')::date
      else null end;
    v_adopted := null;
    v_description := null;
    v_source_stem := public.docket_legislative_source_stem(v_source_url);
    if v_provider_document_id is null or v_source_url is null then continue; end if;
    v_source_document_key := format('text:%s:%s', p_source_bill_id, v_provider_document_id);
    ...
    v_metadata_hash := encode(public.digest(convert_to(v_metadata::text, 'UTF8'), 'sha256'), 'hex');
    insert into public.docket_bill_source_document (
      source_document_key, source_bill_id, provider_document_id, document_family,
      provider_document_type, normalized_version_type, provider_sequence, stage_rank,
      chamber, source_url, provider_url, provider_hash, provider_size, provider_date,
      adopted, description, source_stem, latest_metadata, latest_observed_at
    ) values (
      v_source_document_key, p_source_bill_id, v_provider_document_id, v_document_family,
      v_provider_type, v_version_type, v_sequence::integer, v_stage_rank,
      v_chamber, v_source_url, v_provider_url, v_provider_hash, v_provider_size,
      v_provider_date, v_adopted, v_description, v_source_stem, v_metadata, v_fetched_at
    ...
    ) values (v_source_document_key, v_metadata_hash, v_metadata, v_fetched_at)
  ...
  loop
    ...
    v_provider_type := coalesce(nullif(v_document ->> 'title', ''), 'Amendment');
    ...
    v_adopted := case when v_document ? 'adopted' then (v_document ->> 'adopted')::integer = 1 else null end;
    v_description := nullif(v_document ->> 'description', '');
    v_source_stem := public.docket_legislative_source_stem(v_source_url);
    if v_provider_document_id is null or v_source_url is null then continue; end if;
    v_source_document_key := format('amendment:%s:%s', p_source_bill_id, v_provider_document_id);
    v_metadata := jsonb_build_object(
      'source_bill_id', p_source_bill_id,
      'source_document_key', v_source_document_key,
      'document_family', v_document_family,
  ...
  update public.docket_bill_source_document document
     set predecessor_source_document_key = (
       ...
       order by predecessor.provider_sequence desc, predecessor.provider_document_id desc
       limit 1
     ), updated_at = now()
  ...
  insert into public.civic_genome_bill_version (
    genome_bill_id, source_document_key, source_bill_id, document_family,
    version_type, provider_sequence, stage_rank, chamber,
    version_fingerprint, receipt_json
  )
  select v_genome_bill_id, document.source_document_key, document.source_bill_id,
         document.document_family, document.normalized_version_type,
         document.provider_sequence, document.stage_rank, document.chamber,
  ...
  left join public.civic_genome_bill_version predecessor
    on predecessor.genome_bill_id = v_genome_bill_id
  ...
  left join public.civic_genome_bill_version base
    on base.genome_bill_id = v_genome_bill_id
  ...
  if p_enqueue then
    ...
    select version.bill_version_id, 'eligible',
           ((case when version.document_family = 'amendment'
              then coalesce(base.stage_rank + 50, version.stage_rank)
              else version.stage_rank end) * 1000) + version.provider_sequence,
           now()
  ...
  return jsonb_build_object(
    ...
    'text_version_count', (select count(*) from public.docket_bill_source_document where source_bill_id = p_source_bill_id and document_family = 'text'),
    'amendment_count', (select count(*) from public.docket_bill_source_document where source_bill_id = p_source_bill_id and document_family = 'amendment'),
    'matched_amendment_base_count', (select count(*) from public.docket_bill_source_document where source_bill_id = p_source_bill_id and document_family = 'amendment' and base_source_document_key is not null),
...
begin
  update public.civic_genome_bill_version
     set prism_verification_run_id = new.verification_run_id,
         processing_state = case when new.receipt_count = new.expected_trait_count then 'verified' else 'verification_partial' end,
...
drop trigger if exists civic_genome_version_prism_completion on public.civic_genome_prism_verification_run;
...
revoke all on table public.docket_bill_source_document from public, anon, authenticated;
revoke all on table public.docket_bill_source_document_observation from public, anon, authenticated;
revoke all on table public.civic_genome_bill_version from public, anon, authenticated;
revoke all on table public.civic_genome_legislative_version_queue from public, anon, authenticated;
grant select, insert, update on table public.docket_bill_source_document to service_role;
grant select, insert on table public.docket_bill_source_document_observation to service_role;
grant select, insert, update on table public.civic_genome_bill_version to service_role;
grant select, insert, update on table public.civic_genome_legislative_version_queue to service_role;
revoke all on function public.docket_legislative_version_type(text, text, text) from public, anon, authenticated;
revoke all on function public.docket_legislative_stage_rank(text) from public, anon, authenticated;
revoke all on function public.docket_legislative_source_stem(text) from public, anon, authenticated;
revoke all on function public.guard_civic_genome_bill_version_lineage() from public, anon, authenticated;
revoke all on function public.register_docket_legislative_version_spine(integer, boolean) from public, anon, authenticated;
revoke all on function public.record_civic_genome_version_prism_completion() from public, anon, authenticated;
grant execute on function public.register_docket_legislative_version_spine(integer, boolean) to service_role;

comment on table public.docket_bill_source_document is 'Docket-owned stable identity registry for every official bill text and amendment supplied by the cached legislative provider. Latest metadata is a compatibility mirror; immutable observations are preserved separately.';
comment on table public.docket_bill_source_document_observation is 'Immutable observed metadata receipts for Docket bill texts and amendments. Provider changes create new observations instead of rewriting prior evidence.';
comment on table public.civic_genome_bill_version is 'Civic Genome version-bound operating record linking one Docket source document to its Rosetta extraction, Genome assembly, and Prism verification without converting document versions into separate bill identities.';
comment on table public.civic_genome_legislative_version_queue is 'Durable exact-source queue for deterministic legislative-version decomposition. Each job names one registered Docket document and never selects a latest or preferred version.';
comment on function public.register_docket_legislative_version_spine(integer, boolean) is 'Registers provider-declared bill texts and amendments, preserves observations, binds exact source documents to one Civic Genome bill identity, and optionally queues each version for Rosetta and Prism circulation.';

-- ============================================================================
-- Migration 28 -- exact 2.5.16 generation authorization and replay support.
-- Extends the immutable generation ledger; it does not rewrite or relabel any
-- 2.5.14/2.5.15 authorization, attempt, receipt, binding, or stopped campaign.
-- ============================================================================
...
alter table rosetta_replay.installed_closure_identity
  ...
  check (closure_prefix ~ '^(ctl_|c[1-7]_|v2513_|v2514_|v2515_|v2516_)$');
...
alter table rosetta_replay.replay_campaign
  ...
  check (closure_prefix in ('ctl_','v2513_','v2514_','v2515_','v2516_'));
...
alter table rosetta_replay.candidate_generation_authorization
  ...
  check (closure_prefix in ('v2514_','v2515_','v2516_'));
...
declare v_hash text;
begin
  if p_closure_prefix !~ '^(ctl_|c[1-7]_|v2513_|v2514_|v2515_|v2516_)$' then
...
begin
  ...
  if p_closure_prefix in ('v2514_','v2515_','v2516_')
...
begin
  ...
  where i.closure_sha256=a.closure_hash
    and i.closure_prefix in ('v2514_','v2515_','v2516_');
...
values ('v2516_',rosetta_replay.recompute_closure_sha256('v2516_'))
...
begin
  if not exists(
    ...
    where i.closure_prefix='v2516_'
      and i.closure_sha256=rosetta_replay.recompute_closure_sha256('v2516_')
  ) then
    raise exception 'installed v2516 closure identity differs from current catalog'
      using errcode='P1R14';
  end if;
end;
$block$;
...
set search_path to 'pg_catalog','rosetta_replay' as $fn$
  select p_closure_prefix='ctl_'
    ...
    or (p_closure_prefix in ('v2514_','v2515_','v2516_') and exists(
...
begin
  ...
  if p_closure_prefix not in ('ctl_','v2513_','v2514_','v2515_','v2516_') then
  ...
  if p_closure_prefix in ('v2514_','v2515_','v2516_') and exists(

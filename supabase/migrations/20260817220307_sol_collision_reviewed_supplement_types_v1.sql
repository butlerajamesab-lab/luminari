-- Extend the append-only reviewed-source supplement lane with exact SOL
-- Collision material types. These records remain service-role-only and are
-- never projected to a public Legal Library, Enforcement Hub, Knowledge
-- Backbone, Resource Directory, Civic Map, or Lighthouse action surface.
...
begin
  ...
  perform public.assert_luminari_reviewed_source_supplement_type_v4(v_type);
...
comment on function public.assert_luminari_reviewed_source_supplement_type_v4(text) is
  'Validates prior reviewed supplement types plus exact SOL collision, tolling, preclusion, binding, conflict, blocker, and access-inventory service-only types.';

comment on function public.register_luminari_reviewed_source_supplement_v1(uuid, jsonb, jsonb) is
  'Append-only exact-key reviewed-source supplement registration. All SOL Collision records remain service-only and separately gated.';

comment on table public.luminari_reviewed_source_supplement_revision_v1 is
  'Append-only service-only reviewed source revisions, including exact collision scenarios, route observations and bindings, deadlines, tolling and preclusion assertions, overlaps, fragments, conflicts, blockers, and projection holds. Separate publication is always required.';

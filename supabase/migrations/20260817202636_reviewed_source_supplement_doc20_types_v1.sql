-- Extend the append-only reviewed-source supplement lane with precise DOC20
-- non-route material types. This migration does not publish a route, mutate a
-- stored source row, or delete/retire any historical revision.
--
-- Apply after:
...
--   20260817174500_reviewed_source_supplement_alias_semantics_v2.sql
...
create or replace function public.assert_luminari_reviewed_source_supplement_type_v2(
  p_type text
)
returns void
language plpgsql
...
revoke all on function public.assert_luminari_reviewed_source_supplement_type_v2(text)
  from public, anon, authenticated, service_role;
grant execute on function public.assert_luminari_reviewed_source_supplement_type_v2(text)
  to service_role;

comment on function public.assert_luminari_reviewed_source_supplement_type_v2(text) is
  'Validates precise append-only service-only supplement types, including DOC20 source, program, deadline, relationship, parallel-lens, and held-binding records.';
...
begin
  ...
  perform public.assert_luminari_reviewed_source_supplement_type_v2(v_type);
...
comment on function public.register_luminari_reviewed_source_supplement_v1(uuid, jsonb, jsonb) is
  'Append-only exact-key reviewed-source supplement registration with service-only DOC20 material types and truthful alias semantics.';

comment on table public.luminari_reviewed_source_supplement_revision_v1 is
  'Append-only service-only reviewed source revisions. Includes precise primary resource, program, authority, jurisdiction, deadline, handoff, relationship, parallel-lens, held-binding, integrity, and alias lineage types. Separate publication is always required.';

...
-- Civic Genome temporal supersession v3
--
-- Event-time v2 made source history immutable, but a later source correction
-- could leave both the corrected row and the replaced row canonical. Preserve
-- the immutable ledger while recording every source revision, linking exact
-- replacements, writing deletion tombstones, and deriving current truth only
-- from the latest observed source revision.

create table if not exists public.civic_genome_lifecycle_source_revision_v3 (
  ...
  source_event_keys text[] not null default '{}'::text[],
  event_count integer not null check (event_count >= 0),
  prior_source_revision_id uuid
    references public.civic_genome_lifecycle_source_revision_v3(source_revision_id)
      on delete restrict,
  revision_payload_json jsonb not null default '{}'::jsonb,
  created_at timestamptz not null default now(),
  constraint civic_genome_lifecycle_source_revision_v3_hash_format
    check (source_revision_hash ~ '^[0-9a-f]{64}$'),
  constraint civic_genome_lifecycle_source_revision_v3_payload_hash_format
    check (source_payload_hash ~ '^[0-9a-f]{64}$'),
  constraint civic_genome_lifecycle_source_revision_v3_identity_unique
    unique (genome_bill_id, observed_at, source_revision_hash)
);

create index if not exists civic_genome_lifecycle_source_revision_v3_latest_idx
  on public.civic_genome_lifecycle_source_revision_v3 (
    genome_bill_id,
    observed_at desc,
    created_at desc,
    source_revision_id desc
  );

create index if not exists civic_genome_lifecycle_source_revision_v3_source_idx
  on public.civic_genome_lifecycle_source_revision_v3 (
    source_bill_id,
    observed_at desc
  );

create index if not exists civic_genome_lifecycle_source_revision_v3_prior_idx
  on public.civic_genome_lifecycle_source_revision_v3 (prior_source_revision_id)
  where prior_source_revision_id is not null;

alter table public.civic_genome_lifecycle_source_revision_v3
  enable row level security;
revoke all on table public.civic_genome_lifecycle_source_revision_v3
  from public, anon, authenticated;
grant select on table public.civic_genome_lifecycle_source_revision_v3
  to authenticated, service_role;
grant insert on table public.civic_genome_lifecycle_source_revision_v3
  to service_role;

drop policy if exists civic_genome_lifecycle_source_revision_v3_authenticated_read
  on public.civic_genome_lifecycle_source_revision_v3;
create policy civic_genome_lifecycle_source_revision_v3_authenticated_read
  on public.civic_genome_lifecycle_source_revision_v3
  for select
  to authenticated
  using (true);

drop policy if exists civic_genome_lifecycle_source_revision_v3_service_read
  on public.civic_genome_lifecycle_source_revision_v3;
create policy civic_genome_lifecycle_source_revision_v3_service_read
  on public.civic_genome_lifecycle_source_revision_v3
  for select
  to service_role
  using (true);

drop policy if exists civic_genome_lifecycle_source_revision_v3_service_insert
  on public.civic_genome_lifecycle_source_revision_v3;
create policy civic_genome_lifecycle_source_revision_v3_service_insert
  on public.civic_genome_lifecycle_source_revision_v3
  for insert
  to service_role
  with check (true);

drop trigger if exists civic_genome_lifecycle_source_revision_v3_append_only
  on public.civic_genome_lifecycle_source_revision_v3;
create trigger civic_genome_lifecycle_source_revision_v3_append_only
before update or delete on public.civic_genome_lifecycle_source_revision_v3
for each row execute function public.reject_civic_genome_temporal_history_mutation_v2();

drop trigger if exists civic_genome_lifecycle_source_revision_v3_reject_truncate
  on public.civic_genome_lifecycle_source_revision_v3;
create trigger civic_genome_lifecycle_source_revision_v3_reject_truncate
before truncate on public.civic_genome_lifecycle_source_revision_v3
...
begin
  ...
  with current_history as materialized (
    ...
    where p_source_bill_id is null
       or revision.source_bill_id = p_source_bill_id
    order by
    ...
    where event.event_type <> 'source_tombstone'
      and (p_source_bill_id is null or event.source_bill_id = p_source_bill_id)
      and not exists (
        select 1
        from latest_prior_revision revision
...
comment on function public.sync_civic_genome_lifecycle_history_v3(integer) is
  'Reconciles one or all cached source histories into immutable events, source revisions, correction links, and deletion tombstones.';

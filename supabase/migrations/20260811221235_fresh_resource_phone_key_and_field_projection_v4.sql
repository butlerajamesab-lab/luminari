create or replace function public.luminari_phone_key_v4(p_phone text)
returns text
language plpgsql
immutable
as $$
...
begin
  ...
  foreach v_code in array array['211','311','511','711','811','911','988'] loop
    if p_phone ~ ('(^|[^0-9])'||v_code||'([^0-9]|$)') then return v_code; end if;
  end loop;
  v_match := regexp_match(p_phone,'(^|[^0-9])(1[^0-9]*)?([2-9][0-9]{2})[^0-9]*([0-9]{3})[^0-9]*([0-9]{4})([^0-9]|$)');
  if v_match is null then return null; end if;
  return v_match[3]||v_match[4]||v_match[5];
end;
$$;

create or replace function public.create_luminari_resource_contact_quality_v4(
  p_run_id uuid,
...
begin
  ...
  select q.run_id,p_target_quality_version,q.candidate_key,q.artifact_key,q.artifact_role,q.source_locator,q.effective_name,q.normalized_name_key,
    q.state_code,q.jurisdiction,q.category,q.organization_name,
    case when q.phone is not null and public.luminari_phone_key_v4(q.phone) is not null then q.phone else null end,
    q.email,q.website_url,q.website_domain,
    public.luminari_phone_key_v4(q.phone),
    q.source_record_id,q.source_priority,q.quality_state,
    q.quality_reasons || case when q.phone is not null and public.luminari_phone_key_v4(q.phone) is null then '["discarded_unparseable_phone_contact_v4"]'::jsonb else '[]'::jsonb end,
    now()
  ...
  return jsonb_build_object('run_id',p_run_id,'source_quality_version',p_source_quality_version,'quality_version',p_target_quality_version,
...
$$;
revoke all on function public.create_luminari_resource_contact_quality_v4(uuid,text,text) from public,anon,authenticated;
grant execute on function public.create_luminari_resource_contact_quality_v4(uuid,text,text) to service_role;

create or replace function public.create_luminari_resource_snapshot_v2_3(
...
begin
  ...
  values(p_snapshot_version,p_quality_lanes,'building',false,jsonb_build_object('identity_engine','fresh_resource_identity_v2_3','contact_quality','fresh_resource_contact_quality_v4','field_projection','best_source_attached_field_v1'))
  ...
  with lanes as (
    ...
    where q.quality_state='publishable_candidate' and q.jurisdiction is not null and q.normalized_name_key is not null
  ), exact_stats as (
    ...
    from q0 group by jurisdiction,normalized_name_key
  ), base_stats as (
    ...
    from q0 where base_name_key is not null group by jurisdiction,base_name_key
  ), q1 as (
    select q0.*,
      ...
      case when coalesce(es.exact_distinct_domains,0)>1
    ...
    select q1.*,
      row_number() over(partition by jurisdiction,identity_group_key
        order by lane_priority desc,source_priority desc,(source_record_id is not null) desc,
          (website_domain is not null) desc,(phone_key is not null) desc,artifact_key,candidate_key) as choice_rank
    from q1
  ), grouped as (
    ...
    from ranked group by jurisdiction,identity_group_key
  ), fields as (
    select r.jurisdiction,r.identity_group_key,
      (array_agg(r.phone order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where r.phone is not null))[1] as best_phone,
      (array_agg(r.candidate_key order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where r.phone is not null))[1] as phone_candidate_key,
      (array_agg(r.email order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where r.email is not null))[1] as best_email,
      (array_agg(r.candidate_key order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where r.email is not null))[1] as email_candidate_key,
      (array_agg(r.website_url order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where r.website_url is not null))[1] as best_website,
      (array_agg(r.candidate_key order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where r.website_url is not null))[1] as website_candidate_key,
      (array_agg(c.address order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where c.address is not null
        and c.address !~* '<\/?w:|<xml|</xml'
        and trim(c.address) !~* '^(not published|not available|n/?a\b|none\b|multiple locations|various locations|statewide|see website|contact )'
        and c.address ~* '(P\.?O\.?\s*Box\s+[0-9]+|[0-9]{1,6}\s+.{0,100}\b(street|st\.?|avenue|ave\.?|road|rd\.?|boulevard|blvd|drive|dr\.?|lane|ln\.?|highway|hwy\.?|way|court|ct\.?|parkway|pkwy\.?|place|pl\.?|circle|cir\.?)\b)'))[1] as best_address,
      (array_agg(r.candidate_key order by r.lane_priority desc,r.source_priority desc,r.candidate_key) filter(where c.address is not null
        and c.address !~* '<\/?w:|<xml|</xml'
        and trim(c.address) !~* '^(not published|not available|n/?a\b|none\b|multiple locations|various locations|statewide|see website|contact )'
        and c.address ~* '(P\.?O\.?\s*Box\s+[0-9]+|[0-9]{1,6}\s+.{0,100}\b(street|st\.?|avenue|ave\.?|road|rd\.?|boulevard|blvd|drive|dr\.?|lane|ln\.?|highway|hwy\.?|way|court|ct\.?|parkway|pkwy\.?|place|pl\.?|circle|cir\.?)\b)'))[1] as address_candidate_key
    from ranked r
    join public.luminari_corpus_candidate_v1 c on c.candidate_key=r.candidate_key
    group by r.jurisdiction,r.identity_group_key
  ), resolved as (
    select g.*,
      case
    ...
    select r.*,g.candidate_count,g.candidate_keys,g.source_artifacts,g.observed_domains,g.observed_phones,g.quality_lanes,g.resolution_state,
      f.best_phone,f.phone_candidate_key,f.best_email,f.email_candidate_key,f.best_website,f.website_candidate_key,f.best_address,f.address_candidate_key,
      encode(digest(r.jurisdiction||'|'||r.identity_group_key,'sha256'),'hex') as identity_key
    from ranked r join resolved g using(jurisdiction,identity_group_key)
    join fields f using(jurisdiction,identity_group_key)
  ...
  select v_snapshot_id,public.luminari_resource_identity_uuid_v1(ch.identity_key),ch.identity_key,ch.resolution_state,
    ...
    ch.organization_name,ch.best_phone,ch.best_email,ch.best_website,ch.best_address,
    c.description,c.eligibility_summary,c.apply_notes,
    ...
    jsonb_build_object(
      'identity_engine','fresh_resource_identity_v2_3','contact_quality','fresh_resource_contact_quality_v4','field_projection','best_source_attached_field_v1',
      'identity_group_key',ch.identity_group_key,'base_name_key',ch.base_name_key,
      'exact_strong_conflict',ch.exact_strong_conflict,'exact_distinct_domains',ch.exact_distinct_domains,'exact_distinct_phones',ch.exact_distinct_phones,
      'canonical_candidate_key',ch.candidate_key,'canonical_artifact_key',ch.artifact_key,'source_locator',ch.source_locator,
      'phone_candidate_key',ch.phone_candidate_key,'email_candidate_key',ch.email_candidate_key,'website_candidate_key',ch.website_candidate_key,'address_candidate_key',ch.address_candidate_key,
      'quality_reasons',ch.quality_reasons,'candidate_state',c.candidate_state,'parser_version',c.parser_version,
      'source_content_sha256',c.source_content_sha256,'jurisdiction_resolution_state',c.jurisdiction_resolution_state
    ),
    encode(digest(ch.identity_key||'|'||ch.resolution_state||'|'||ch.candidate_keys::text||'|'||ch.source_artifacts::text||'|'||ch.quality_lanes::text||'|'||coalesce(ch.phone_candidate_key,'')||'|'||coalesce(ch.email_candidate_key,'')||'|'||coalesce(ch.website_candidate_key,'')||'|'||coalesce(ch.address_candidate_key,''),'sha256'),'hex')
  from chosen ch join public.luminari_corpus_candidate_v1 c on c.candidate_key=ch.candidate_key;
...
$$;
revoke all on function public.create_luminari_resource_snapshot_v2_3(text,jsonb) from public,anon,authenticated;
grant execute on function public.create_luminari_resource_snapshot_v2_3(text,jsonb) to service_role;

...
with staged as (
  ...
  select
    s.payload->>'resource_id',
    'domain_deep_dive_v3_13_stage',
    s.stage_row_id::text,
    s.source_hash_raw,
    s.payload->>'organization_name',
    s.payload->>'organization_type',
    s.payload->>'resource_category',
    s.payload->>'subcategory',
    s.payload->>'jurisdiction',
    s.payload->>'jurisdiction_level',
    s.payload->>'notes',
    s.payload->>'notes',
    array_remove(array[s.payload->>'service_type'],null),
    jsonb_build_array(s.payload->>'resource_category'),
    s.payload || jsonb_build_object(
      'source_file',s.source_file,
      'source_row_key',s.source_row_key,
      'source_hash_raw',s.source_hash_raw,
      'verbatim_source_text',s.payload->>'notes'
    ),
    lower(coalesce(s.payload->>'verification_status','verified')),
    'promoted',
    'verified'
  from staged s
...
set disposition='insert',
    target_table='luminari_resource_entities',
    target_identity=jsonb_build_object(
      'canonical_id',s.payload->>'resource_id',
      'resource_name',s.payload->>'organization_name'
    ),
...
where d.source_file=s.source_file
  and d.source_row_key=s.source_row_key

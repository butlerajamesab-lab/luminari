create or replace function public.create_luminari_resource_contact_quality_v3(
  p_run_id uuid,
...
begin
  ...
  select
    q.run_id,p_target_quality_version,q.candidate_key,q.artifact_key,q.artifact_role,q.source_locator,q.effective_name,q.normalized_name_key,
    q.state_code,q.jurisdiction,q.category,q.organization_name,
    case
      when coalesce(q.phone,'') ~* '<\/?w:|<xml|</xml' then null
      when trim(coalesce(q.phone,'')) ~* '^(not published|not available|n/?a\b|none\b|use website|see website|online only)' then null
      when length(regexp_replace(coalesce(q.phone,''),'\D','','g')) < 7
           and coalesce(q.phone,'') !~* '(^|[^0-9])211([^0-9]|$)' then null
      else nullif(trim(q.phone),'') end,
    case
      when trim(coalesce(q.email,'')) ~* '^[A-Z0-9._%+\-]+@[A-Z0-9.\-]+\.[A-Z]{2,}$' then trim(q.email)
      else null end,
    case
      when coalesce(q.website_url,'') ~* '<\/?w:|<xml|</xml' then null
      when coalesce(q.website_url,'') ~* '(https?://|www\.|[a-z0-9-]+\.[a-z]{2,})' then q.website_url
      else null end,
    case
      when coalesce(q.website_url,'') ~* '<\/?w:|<xml|</xml' then null
      when coalesce(q.website_url,'') ~* '(https?://|www\.|[a-z0-9-]+\.[a-z]{2,})'
        then nullif(lower(regexp_replace(coalesce(substring(q.website_url from '(?i)(?:https?://)?(?:www\.)?([^/\s]+)'),''),'^www\.','','i')),'')
      else null end,
    case
      when coalesce(q.phone,'') ~* '<\/?w:|<xml|</xml' then null
      when trim(coalesce(q.phone,'')) ~* '^(not published|not available|n/?a\b|none\b|use website|see website|online only)' then null
      when length(regexp_replace(coalesce(q.phone,''),'\D','','g')) >= 10 then right(regexp_replace(q.phone,'\D','','g'),10)
      else null end,
    q.source_record_id,q.source_priority,q.quality_state,
    q.quality_reasons
      || case when q.email is not null and trim(q.email) !~* '^[A-Z0-9._%+\-]+@[A-Z0-9.\-]+\.[A-Z]{2,}$' then '["discarded_nonemail_contact_value"]'::jsonb else '[]'::jsonb end
      || case when q.phone is not null and (q.phone ~* '<\/?w:|<xml|</xml') then '["discarded_xml_contaminated_phone"]'::jsonb else '[]'::jsonb end
      || case when q.phone is not null and q.phone !~* '<\/?w:|<xml|</xml' and length(regexp_replace(q.phone,'\D','','g')) < 7 and q.phone !~* '(^|[^0-9])211([^0-9]|$)' then '["discarded_nonphone_contact_value"]'::jsonb else '[]'::jsonb end
      || case when q.website_url is not null and (q.website_url ~* '<\/?w:|<xml|</xml' or q.website_url !~* '(https?://|www\.|[a-z0-9-]+\.[a-z]{2,})') then '["discarded_nonwebsite_contact_value"]'::jsonb else '[]'::jsonb end,
    now()
...
$$;
revoke all on function public.create_luminari_resource_contact_quality_v3(uuid,text,text) from public,anon,authenticated;
grant execute on function public.create_luminari_resource_contact_quality_v3(uuid,text,text) to service_role;

create or replace function public.create_luminari_resource_snapshot_v2_2(
...
begin
  ...
  values(p_snapshot_version,p_quality_lanes,'building',false,jsonb_build_object('identity_engine','fresh_resource_identity_v2_2','contact_quality','fresh_resource_contact_quality_v3'))
  ...
  select v_snapshot_id,public.luminari_resource_identity_uuid_v1(ch.identity_key),ch.identity_key,ch.resolution_state,
    ...
    ch.organization_name,ch.phone,ch.email,ch.website_url,
    case
      when coalesce(c.address,'') ~* '<\/?w:|<xml|</xml' then null
      when trim(coalesce(c.address,'')) ~* '^(not published|not available|n/?a\b|none\b|multiple locations|various locations|statewide|see website|contact )' then null
      when c.address ~* '(P\.?O\.?\s*Box\s+[0-9]+|[0-9]{1,6}\s+.{0,100}\b(street|st\.?|avenue|ave\.?|road|rd\.?|boulevard|blvd|drive|dr\.?|lane|ln\.?|highway|hwy\.?|way|court|ct\.?|parkway|pkwy\.?|place|pl\.?|circle|cir\.?)\b)' then c.address
      else null end,
    ...
    jsonb_build_object(
      'identity_engine','fresh_resource_identity_v2_2','contact_quality','fresh_resource_contact_quality_v3',
...
$$;
revoke all on function public.create_luminari_resource_snapshot_v2_2(text,jsonb) from public,anon,authenticated;
grant execute on function public.create_luminari_resource_snapshot_v2_2(text,jsonb) to service_role;

create or replace function public.create_luminari_resource_normalized_quality_v2(
  p_run_id uuid,
...
begin
  ...
  select q.run_id,p_target_quality_version,q.candidate_key,q.artifact_key,q.artifact_role,q.source_locator,q.effective_name,
    nullif(regexp_replace(lower(coalesce(q.effective_name,'')),'[^a-z0-9]+','','g'),''),
    q.state_code,q.jurisdiction,q.category,q.organization_name,q.phone,q.email,q.website_url,q.website_domain,q.phone_key,
    q.source_record_id,q.source_priority,q.quality_state,
    q.quality_reasons || '["normalization_lower_before_ascii_filter_v2"]'::jsonb,now()
...
$$;
revoke all on function public.create_luminari_resource_normalized_quality_v2(uuid,text,text) from public,anon,authenticated;
grant execute on function public.create_luminari_resource_normalized_quality_v2(uuid,text,text) to service_role;

create or replace function public.create_luminari_resource_snapshot_v2(
...
begin
  ...
  values(p_snapshot_version,p_quality_lanes,'building',false,jsonb_build_object('identity_engine','fresh_resource_identity_v2'))
  ...
  with lanes as (
    ...
    select jurisdiction,base_name_key,
      ...
      min(website_domain) filter(where website_domain is not null) as only_domain,
      min(phone_key) filter(where phone_key is not null) as only_phone
    from q0
    ...
    select q0.*,
      case
        ...
        when coalesce(bs.distinct_domains,0)=1 then 'domain:'||q0.base_name_key||':'||bs.only_domain
        when coalesce(bs.distinct_domains,0)=0 and coalesce(bs.distinct_phones,0)=1 then 'phone:'||q0.base_name_key||':'||bs.only_phone
  ...
  select v_snapshot_id,public.luminari_resource_identity_uuid_v1(ch.identity_key),ch.identity_key,ch.resolution_state,
    ...
    jsonb_build_object(
      'identity_engine','fresh_resource_identity_v2',
  ...
  return jsonb_build_object('snapshot_id',v_snapshot_id,'snapshot_version',p_snapshot_version,'status','sealed',
    'identity_engine','fresh_resource_identity_v2','resolved_resources',v_resource_count,'identity_conflicts',v_conflict_count,
    'receipt_hash',v_receipt_hash,'activated',false);
end;
$$;
revoke all on function public.create_luminari_resource_snapshot_v2(text,jsonb) from public,anon,authenticated;
grant execute on function public.create_luminari_resource_snapshot_v2(text,jsonb) to service_role;

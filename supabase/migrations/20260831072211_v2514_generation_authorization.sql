-- ============================================================================
-- Migration 23 -- exact 2.5.14 generation authorization and replay support.
--
-- The installed 2.5.13 closure and its source-level authorization remain
-- immutable.  This migration captures the separately installed 2.5.14
-- closure identity and requires a new authorization bound to the exact
-- source, snapshot, engine, rule set, prefix, and closure hash.
-- ============================================================================
...
alter table rosetta_replay.installed_closure_identity
  ...
  check (closure_prefix ~ '^(ctl_|c[1-7]_|v2513_|v2514_)$');
...
alter table rosetta_replay.replay_campaign
  ...
  check (closure_prefix in ('ctl_','v2513_','v2514_'));
...
declare v_hash text;
begin
  if p_closure_prefix !~ '^(ctl_|c[1-7]_|v2513_|v2514_)$' then
...
create table if not exists rosetta_replay.candidate_generation_authorization (
    ...
    closure_prefix text not null check(closure_prefix='v2514_'),
    closure_hash text not null check(closure_hash~'^[0-9a-f]{64}$'),
    authorization_scope text not null
      check(authorization_scope='full_candidate_generation'),
    ...
    primary key(source_registry_id,engine_version,rule_set_version,
                closure_prefix,closure_hash)
);

comment on table rosetta_replay.candidate_generation_authorization is
  'Immutable authorization bound to an exact source, snapshot, candidate identity, and captured closure hash. It does not alter or inherit the 2.5.13 authorization.';

create or replace function rosetta_replay.reject_candidate_generation_authorization_mutation()
returns trigger language plpgsql set search_path to 'pg_catalog' as $fn$
begin
  raise exception 'candidate_generation_authorization_is_immutable'
    using errcode='raise_exception';
end;
$fn$;

drop trigger if exists candidate_generation_authorization_immutable
  on rosetta_replay.candidate_generation_authorization;
create trigger candidate_generation_authorization_immutable before update or delete
on rosetta_replay.candidate_generation_authorization for each row
execute function rosetta_replay.reject_candidate_generation_authorization_mutation();

create or replace function rosetta_replay.candidate_generation_is_authorized(
    p_source_registry_id uuid,p_closure_prefix text,p_engine_version text,
    p_rule_set_version text,p_closure_hash text)
...
set search_path to 'pg_catalog','rosetta_replay' as $fn$
  select exists(
    ...
    where a.source_registry_id=p_source_registry_id
      ...
      and a.closure_hash=p_closure_hash
...
begin
  ...
  if p_closure_prefix='v2514_' and not v_generation_authorized then
    raise exception '2.5.14 claim lacks exact candidate generation authorization for source %',
...
declare
  ...
  v_candidate_authorized boolean;v_generation_authorized boolean;
begin
  ...
  v_generation_authorized:=
    rosetta_replay.candidate_generation_is_authorized(
      a.source_registry_id,'v2514_',a.engine_version,a.rule_set_version,
      a.closure_hash);

  if a.closure_hash=rosetta_replay.closure_sha256('v2514_')
     and not v_generation_authorized then
    raise exception '2.5.14 finalization lacks exact candidate generation authorization'
      using errcode='P1R28';
  end if;
  if not v_has_expectation and not (
...
values ('v2514_',rosetta_replay.recompute_closure_sha256('v2514_'))
...
begin
  if not exists(
    ...
    where i.closure_prefix='v2514_'
      and i.closure_sha256=rosetta_replay.recompute_closure_sha256('v2514_')
  ) then
    raise exception 'installed v2514 closure identity differs from current catalog'
      using errcode='P1R14';
  end if;
end;
$block$;
...
begin
  ...
  if p_closure_prefix is distinct from 'v2514_'
     or p_engine_version is distinct from 'rosetta-v3-deterministic-sql-2.5.14'
     or p_rule_set_version is distinct from
        'rosetta-five-layer-structural-correctness-2.5.14' then
    raise exception 'authorization must name the exact 2.5.14 candidate identity'
      using errcode='P1R12';
  end if;
...
set search_path to 'pg_catalog','rosetta_replay' as $fn$
  select p_closure_prefix='ctl_'
    ...
    or (p_closure_prefix='v2513_' and exists(
      ...
      where a.source_registry_id=p_source_registry_id))
    or (p_closure_prefix='v2514_' and exists(
...
begin
  ...
  for i in 1..p_executor_count loop
    v_name:='rosetta-'||rtrim(p_closure_prefix,'_')||'-'||v_id::text
      ||'-execute-'||i::text;
    v_command:=format(
      'select rosetta_replay.replay_campaign_execute_next(%L::uuid)',
      v_id::text);
    execute 'select cron.schedule($1,$2,$3)'
      into v_job using v_name,'5 seconds',v_command;
    v_jobs:=array_append(v_jobs,v_job);
...
revoke all on table rosetta_replay.candidate_generation_authorization
  from public,anon,authenticated,service_role;
revoke all on function rosetta_replay.recompute_closure_sha256(text)
  from public,anon,authenticated,service_role;
revoke all on function rosetta_replay.reject_candidate_generation_authorization_mutation()
  from public,anon,authenticated,service_role;
revoke all on function rosetta_replay.candidate_generation_is_authorized(

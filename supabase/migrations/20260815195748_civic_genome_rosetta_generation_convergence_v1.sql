...
queue_counts as (
  select
    count(*) filter(where queue_state='eligible')::integer eligible_count,
    count(*) filter(where queue_state='running')::integer running_count,
    count(*) filter(where queue_state='retry')::integer retry_count,
    count(*) filter(where queue_state='completed')::integer completed_count,
    count(*) filter(where queue_state='dead_letter')::integer dead_letter_count
...
select
  ...
  'rosetta-v3-deterministic-sql-2.5.3'::text target_engine_version,
  'rosetta-five-layer-structural-correctness-2.5.3'::text target_rule_set_version,

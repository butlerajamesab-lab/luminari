-- Review hardening for Lighthouse's Atlas Domain 3 semantic-currentness boundary.
-- Production migration version: 20260812084726.
--
-- - aligns cross-category semantic identity with Atlas by excluding the
--   incidental primary stream for atlas.domain3.cross_category_entity;
-- - serializes registrations per semantic pattern;
-- - restores conflict-safe exact replay insertion;
-- - preserves the explicit supersedes_id contract for legacy writers while
--   rejecting ambiguous or self-referential predecessor requests.

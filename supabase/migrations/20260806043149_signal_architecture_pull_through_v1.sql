...
create table if not exists public.signal_domain3_source_classification_v1 (
  ...
  constraint signal_domain3_source_class_output_check check (
    output_class in ('systemic_candidate','observation_only','operational_only','unsupported_rule')
  ),
  ...
  constraint signal_domain3_source_class_hash_check check (rule_hash ~ '^[0-9a-f]{64}$'),
  constraint signal_domain3_source_class_unique unique (
    classifier_id,classifier_version,source_signal_type,source_rule_id,source_rule_version,source_view
  )
);
...
with declared_rules as (
  select * from (values
    ('classification_activity','openstates_wa_classification_activity_v1','v1','public.v_civic_map_signals_production','observation_only',false,'A provider classification observation is source material. It is not statistical evidence of systemic activity by itself.'),
    ('jurisdiction_legislative_activity','openstates_wa_jurisdiction_legislative_activity_v1','v1','public.v_civic_map_signals_production','observation_only',false,'A legislative observation identifies activity in a jurisdiction. It does not establish a systemic live-data signal.'),
    ('new_statute_or_bill','openstates_wa_new_statute_or_bill_v1','v1','public.v_civic_map_signals_production','observation_only',false,'The existence of a new bill or statute is an Atlas observation and Docket input, not a Domain 3 systemic signal.'),
    ('stream_health_alert','post_investigation_prime_pattern_bridge_v1','v1','public.v_civic_map_signals_production','operational_only',false,'Stream health describes pipeline operation. It must not be represented as civic harm, actor misconduct, or systemic failure.')
  ) as rule(source_signal_type,source_rule_id,source_rule_version,source_view,output_class,eligible_for_canonical_registration,rationale)
), receipts as (
  select rule.*,
    public.signal_architecture_hash_v1(jsonb_build_object(
      'classifier_id','signal_domain3_source_classifier','classifier_version','1.0.0',
      'source_signal_type',rule.source_signal_type,'source_rule_id',rule.source_rule_id,
      'source_rule_version',rule.source_rule_version,'source_view',rule.source_view,
...
insert into public.signal_domain3_source_classification_v1 (
  classifier_id,classifier_version,source_signal_type,source_rule_id,source_rule_version,
  source_view,output_class,eligible_for_canonical_registration,rationale,rule_hash
)
select 'signal_domain3_source_classifier','1.0.0',source_signal_type,source_rule_id,
       source_rule_version,source_view,output_class,eligible_for_canonical_registration,
       rationale,rule_hash
from receipts
on conflict (classifier_id,classifier_version,source_signal_type,source_rule_id,source_rule_version,source_view)
do nothing;

create or replace view public.v_atlas_domain3_signal_candidates_v1
with (security_invoker = true) as
select
  bridge.bridge_record_id,bridge.atlas_signal_id,bridge.signal_type,bridge.source_system,
  bridge.bridge_version,bridge.source_connector_id,bridge.raw_record_id,bridge.statute_id,
  bridge.entity_ids,bridge.jurisdiction_raw_value,bridge.jurisdiction_id,bridge.source_url,
  bridge.detected_at,bridge.bridged_at,bridge.confidence_score,bridge.severity,
  bridge.signal_status,bridge.rule_id,bridge.rule_version,bridge.generation_method,
  bridge.record_origin,bridge.verification_status,bridge.source_view,
  ...
  coalesce(classification.rationale,
  ...
  public.signal_architecture_hash_v1(jsonb_build_object(
    'contract','atlas_domain3_signal_candidate.v1','bridge_record_id',bridge.bridge_record_id,
    'atlas_signal_id',bridge.atlas_signal_id,'signal_type',bridge.signal_type,
    'source_rule_id',bridge.rule_id,'source_rule_version',bridge.rule_version,
...
with domain_counts as (
  select domain_code,domain_label,canonical_relation,source_owner,total_record_count,
         current_record_count,latest_record_at
  from public.v_signal_architecture_summary
), atlas_classification as (
  select source_class,count(*)::bigint as record_count,
         count(*) filter (where eligible_for_canonical_registration)::bigint as eligible_record_count,
...
select jsonb_build_object(
  ...
  'source_domains',coalesce((
    select jsonb_agg(jsonb_build_object(
      'domain_code',domain_code,'domain_label',domain_label,
      'canonical_relation',canonical_relation,'source_owner',source_owner,
      'total_record_count',total_record_count,'current_record_count',current_record_count,
      'latest_record_at',latest_record_at
    ) order by case domain_code when 'case_intake' then 1 when 'legal_pattern' then 2 when 'live_data' then 3 else 4 end)
    from domain_counts
  ),'[]'::jsonb),
  'atlas_bridge',jsonb_build_object(
    'verified_transport_rows',(select count(*)::bigint from public.v_atlas_lighthouse_bridge_v1_verified),
    'classifications',coalesce((
      select jsonb_agg(jsonb_build_object(
        'source_class',source_class,'record_count',record_count,
        'eligible_record_count',eligible_record_count,'latest_detected_at',latest_detected_at
      ) order by source_class)
      from atlas_classification
    ),'[]'::jsonb),
    'eligible_systemic_candidates',(
      select count(*)::bigint from public.v_atlas_domain3_signal_candidates_v1
...
declare v_inventory jsonb; v_semantic_body jsonb;
begin
  select inventory into v_inventory from public.v_signal_pull_through_inventory_v1;
  v_semantic_body := jsonb_build_object(
    'contract_id',v_inventory->>'contract_id','contract_version',v_inventory->>'contract_version',
    'source_domains',v_inventory->'source_domains','atlas_bridge',v_inventory->'atlas_bridge',
...
revoke all on table public.signal_domain3_source_classification_v1 from public,anon,authenticated;
revoke all on table public.v_atlas_domain3_signal_candidates_v1 from public,anon,authenticated;
revoke all on table public.v_signal_pull_through_inventory_v1 from public,anon,authenticated;
revoke all on function public.get_signal_pull_through_snapshot_v1() from public,anon,authenticated;
grant select on table public.signal_domain3_source_classification_v1 to service_role;
grant select on table public.v_atlas_domain3_signal_candidates_v1 to service_role;
grant select on table public.v_signal_pull_through_inventory_v1 to service_role;
grant execute on function public.get_signal_pull_through_snapshot_v1() to service_role;
...
comment on view public.v_signal_pull_through_inventory_v1 is
  'Service-role restricted summary of the three canonical source domains, Atlas source classifications, legacy quarantine, and convergence readiness without exposing intake details.';

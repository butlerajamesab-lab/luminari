    ...
    select
      case
        when run.run_status in ('completed','validated')
         ...
         and nullif(regexp_replace(run.engine_version, '[[:space:]]', '', 'g'), '') is not null
         and nullif(regexp_replace(run.rule_set_version, '[[:space:]]', '', 'g'), '') is not null
         and nullif(regexp_replace(run.rule_manifest_hash, '[[:space:]]', '', 'g'), '') is not null
         and nullif(regexp_replace(run.configuration_hash, '[[:space:]]', '', 'g'), '') is not null
         and nullif(regexp_replace(run.source_identity_hash, '[[:space:]]', '', 'g'), '') is not null
         and nullif(regexp_replace(run.source_content_hash, '[[:space:]]', '', 'g'), '') is not null
         and nullif(regexp_replace(run.output_content_hash, '[[:space:]]', '', 'g'), '') is not null
          then case
        ...
        when run.run_status = 'in_progress'
         ...
         and nullif(regexp_replace(run.engine_version, '[[:space:]]', '', 'g'), '') is null
        ...
        when run.run_status in ('completed','validated')
         ...
         and run.source_content_id is null
         and nullif(regexp_replace(run.engine_version, '[[:space:]]', '', 'g'), '') is null
         and nullif(regexp_replace(run.output_content_hash, '[[:space:]]', '', 'g'), '') is null

-- ============================================================================
-- Migration: lane c4 -- C4 occurrence-aware span binding and credible reference-date gate
-- One-variable experiment: a full independent copy of the 51-function closure
-- in rosetta_v2513 with prefix c4_, identity tokens swapped inside string
-- literals only ('2.5.11' -> '2.5.13-c4'), plus the lane's surgical change.
...
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_blocking_structural_repair_count(p_extraction_run_id integer)
 RETURNS integer
 LANGUAGE sql
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_normalize_clause_text(p_text text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE STRICT
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v24_amendment_operations(p_source_text text)
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v24_prune_amendment_projection(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2510_amendment_operations(p_source_text text)
 RETURNS TABLE(operation_ordinal integer, operation_text text, target_locator text, operation_kind text, char_offset_start integer, char_offset_end integer)
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  select
    ...
    rosetta_v2513.c4_rosetta_v2510_clean_amendment_operation_text(operation.operation_text) as operation_text,
    operation.target_locator,
    operation.operation_kind,
    operation.char_offset_start,
    operation.char_offset_start
      + char_length(rosetta_v2513.c4_rosetta_v2510_clean_amendment_operation_text(operation.operation_text)) as char_offset_end
  from rosetta_v2513.c4_rosetta_v24_amendment_operations(p_source_text) operation
  where nullif(btrim(rosetta_v2513.c4_rosetta_v2510_clean_amendment_operation_text(operation.operation_text)), '') is not null
  order by operation.operation_ordinal;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2510_clean_amendment_operation_text(p_operation_text text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_amendment_disposition(p_source_text text, p_source_metadata jsonb)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  if v_metadata_disposition is not null
    ...
    raise exception using
      errcode='22000',
      message='rosetta_v2513c4_amendment_disposition_conflict',
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_amendment_format(p_source_text text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  select case
    when exists (
      select 1
      from rosetta_v2513.c4_rosetta_v2510_amendment_operations(p_source_text)
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_amendment_operations(p_source_text text)
 RETURNS TABLE(operation_ordinal integer, operation_text text, target_locator text, operation_kind text, char_offset_start integer, char_offset_end integer)
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  select
    operation.operation_ordinal,
    rosetta_v2513.c4_rosetta_v2511_clean_amendment_operation_text(operation.operation_text) as operation_text,
    operation.target_locator,
    operation.operation_kind,
    operation.char_offset_start,
    operation.char_offset_start
      + char_length(rosetta_v2513.c4_rosetta_v2511_clean_amendment_operation_text(operation.operation_text)) as char_offset_end
  from rosetta_v2513.c4_rosetta_v24_amendment_operations(p_source_text) operation
  where nullif(btrim(rosetta_v2513.c4_rosetta_v2511_clean_amendment_operation_text(operation.operation_text)), '') is not null
  order by operation.operation_ordinal;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_canonical_output(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE sql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  with base as (
    select rosetta_v2513.c4_rosetta_v254_canonical_output(p_extraction_run_id) as value
  ),
  structural as (
  ...
  select case when base.value is null then null else
    base.value || jsonb_build_object(
      'contract','rosetta-canonical-law-view-v2513c4',
  ...
  cross join counts;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_clean_amendment_operation_text(p_operation_text text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_final_coverage(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE sql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_finalize_extraction(p_extraction_run_id integer, p_source_text text, p_source_metadata jsonb, p_base_receipt jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if v_run.engine_version <> 'rosetta-v3-deterministic-sql-2.5.13-c4'
     or v_run.rule_set_version <> 'rosetta-five-layer-structural-correctness-2.5.13-c4' then
  ...
  if v_document_family = 'amendment' then
    perform rosetta_v2513.c4_rosetta_v24_prune_amendment_projection(p_extraction_run_id);
  end if;

  update rosetta_v2513.term_definition definition
     set definition_text = rosetta_v2513.c4_rosetta_v25_exact_definition_text(
       p_source_text,
       definition.definition_text
     )
   where definition.extraction_run_id = p_extraction_run_id
     and definition.definition_text is distinct from
       rosetta_v2513.c4_rosetta_v25_exact_definition_text(
         p_source_text,
         definition.definition_text
       );
  ...
  from rosetta_v2513.term_definition definition
  where definition.extraction_run_id = p_extraction_run_id
    and not rosetta_v2513.c4_rosetta_v25_projected_contains(p_source_text, definition.definition_text);
  ...
  if v_document_family = 'amendment' then
    ...
    for v_operation in
      select *
      from rosetta_v2513.c4_rosetta_v2511_amendment_operations(p_source_text)
    ...
    loop
      ...
      v_block_id := 'blk-v2513c4-' || v_run.source_identity_hash || '-' || v_run.configuration_hash || '-amend-' ||
        lpad(v_operation.operation_ordinal::text, 4, '0');
      v_override_id := 'ov-v2513c4-' || v_run.source_identity_hash || '-' || v_run.configuration_hash || '-amend-' ||
        lpad(v_operation.operation_ordinal::text, 4, '0');
      ...
      insert into rosetta_v2513.entity_override (
        ...
        null,
        null,
        rosetta_v2513.c4_rosetta_v2511_amendment_disposition(p_source_text, p_source_metadata),
        1.00,
        'confirmed'
      ) on conflict (id) do nothing;
      ...
      select
        'lc-v2513c4-' || v_run.source_identity_hash || '-' || v_run.configuration_hash || '-amend-' ||
    ...
    if v_operation_count = 0
       and rosetta_v2513.c4_rosetta_v2511_amendment_format(p_source_text) <> 'marked_full_text_reprint' then
      raise exception using
        errcode = '22000',
        message = 'rosetta_v2513c4_amendment_structure_not_recognized';
    end if;
    ...
    where operation.extraction_run_id = p_extraction_run_id
      ...
      and (
        strpos(
          lower(rosetta_v2513.c4_rosetta_v2_normalize_text(p_source_text)),
          lower(rosetta_v2513.c4_rosetta_v2_normalize_text(operation.override_scope))
        ) = 0
  ...
  v_output := rosetta_v2513.c4_rosetta_v2511_canonical_output(p_extraction_run_id);
  if v_output is null then
    raise exception 'rosetta_v2513c4_canonical_output_unavailable';
  end if;
  ...
  v_validation := jsonb_build_object(
    'status', 'pass',
    'contract', 'rosetta-structural-correctness-v2513c4',
    ...
    'amendment_format', case when v_document_family = 'amendment' then rosetta_v2513.c4_rosetta_v2511_amendment_format(p_source_text) else null end,
    'amendment_disposition', case when v_document_family = 'amendment'
      then rosetta_v2513.c4_rosetta_v2511_amendment_disposition(p_source_text, p_source_metadata)
      else null end
  );
  ...
  insert into rosetta_v2513.validation_result (
    ...
    'vr-v2513c4-' || v_run.source_identity_hash || '-' || v_run.configuration_hash || '-exact-source-structure',
    p_extraction_run_id,
    'exact_source_structure_v2513c4',
    'pass',
    0,
    v_validation
  ...
  update rosetta_v2513.extraction_manifest
     set row_counts = v_row_counts,
         validation_results = coalesce(validation_results, '{}'::jsonb)
           || jsonb_build_object('exact_source_structure_v2513c4', v_validation),
  ...
  return p_base_receipt || jsonb_build_object(
    'engine_version', 'rosetta-v3-deterministic-sql-2.5.13-c4',
    'rule_set_version', 'rosetta-five-layer-structural-correctness-2.5.13-c4',
    'output_content_hash', v_output_hash,
    'row_counts', v_row_counts,
    'exact_source_structure_v2513c4', v_validation,
    'replayed', coalesce((p_base_receipt ->> 'replayed')::boolean, false)
  );
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_reclassify_amendment_structure(p_extraction_run_id integer, p_source_text text, p_source_metadata jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
  ...
  if not found then raise exception 'rosetta_v2513c4_extraction_run_not_found'; end if;
  ...
  if v_family is distinct from 'amendment' then
    return jsonb_build_object('contract','rosetta-amendment-structural-representation-v2513c4','document_family',nullif(v_family,''),'applied',false,'representation_count',0);
  end if;

  v_disposition:=rosetta_v2513.c4_rosetta_v2511_amendment_disposition(p_source_text,p_source_metadata);
  v_prune:=rosetta_v2513.c4_rosetta_v24_prune_amendment_projection(p_extraction_run_id);

  for v_operation in select * from rosetta_v2513.c4_rosetta_v2511_amendment_operations(p_source_text) order by operation_ordinal loop
    v_operation_count:=v_operation_count+1;
    v_block_id:='blk-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-amend-'||lpad(v_operation.operation_ordinal::text,4,'0');
    v_representation_id:='sr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-amend-'||lpad(v_operation.operation_ordinal::text,4,'0');
    ...
    if not found then raise exception using errcode='22000',message='rosetta_v2513c4_amendment_operation_block_missing',detail=v_block_id; end if;
  ...
  if v_operation_count=0
     and rosetta_v2513.c4_rosetta_v2511_amendment_format(p_source_text) <> 'marked_full_text_reprint' then
    raise exception 'rosetta_v2513c4_amendment_structure_not_recognized';
  end if;
  ...
  select
    'lc-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-nonop-'
  ...
  return jsonb_build_object(
    'contract','rosetta-amendment-structural-representation-v2513c4',
    'document_family','amendment','applied',true,'amendment_disposition',v_disposition,
    'representation_count',v_operation_count,'amendment_format',rosetta_v2513.c4_rosetta_v2511_amendment_format(p_source_text),'operative_layer_projection','not_applied',
    'coverage_block_scope','document_section_and_amendment_operation','prune_receipt',v_prune
  );
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_reconcile_structural_correctness(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
  v_base:=rosetta_v2513.c4_rosetta_v254_reconcile_structural_correctness(p_extraction_run_id);
  return v_base||jsonb_build_object('contract','rosetta-structural-reconciliation-v2513c4');
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_refresh_final_coverage_receipts(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  if not found then raise exception 'rosetta_v2513c4_extraction_run_not_found'; end if;
  v_coverage:=rosetta_v2513.c4_rosetta_v2511_final_coverage(p_extraction_run_id);
  ...
  v_details:=jsonb_build_object('contract','rosetta-final-five-layer-coverage-v2513c4','coverage',v_coverage,'layer_count',v_layer_count,'terminal',v_terminal);

  insert into rosetta_v2513.validation_result(id,extraction_run_id,test_name,test_result,failure_count,details)
  values('vr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-coverage-final',p_extraction_run_id,'five_layer_coverage',case when v_terminal then 'pass' else 'fail' end,case when v_terminal then 0 else 1 end,v_details)
  ...
  insert into rosetta_v2513.validation_result(id,extraction_run_id,test_name,test_result,failure_count,details)
  values('vr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-no-pending-final',p_extraction_run_id,'no_pending_coverage',case when v_terminal then 'pass' else 'fail' end,case when v_terminal then 0 else 1 end,v_details)
  on conflict(extraction_run_id,test_name) do update set test_result=excluded.test_result,failure_count=excluded.failure_count,details=excluded.details,executed_at=now();

  return v_details||jsonb_build_object('status',case when v_terminal then 'pass' else 'fail' end);
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_validate_extraction(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
declare
  ...
  v_status text;
begin
  v_base:=rosetta_v2513.c4_rosetta_v25_validate_extraction(p_extraction_run_id,p_source_text);
  select * into v_run from rosetta_v2513.extraction_run where id=p_extraction_run_id;
  if not found then raise exception 'rosetta_v2513c4_extraction_run_not_found'; end if;
  ...
  if v_family='amendment' then
    ...
    return v_base||jsonb_build_object(
      'status',v_status,
      'engine_contract','rosetta-structural-self-check-v2513c4-amendment-projection-aware',
  ...
  return v_base||jsonb_build_object(
    'engine_contract','rosetta-structural-self-check-v2513c4',
    'document_family',nullif(v_family,'')
  );
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2511_validate_independent_structure(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
declare
  ...
  v_status text;
begin
  v_base:=rosetta_v2513.c4_rosetta_v253_validate_independent_structure(p_extraction_run_id,p_source_text);
  select * into v_run from rosetta_v2513.extraction_run where id=p_extraction_run_id;
  if not found then raise exception 'rosetta_v2513c4_extraction_run_not_found'; end if;
  if v_run.source_content_id is not null then
    select coalesce(source_metadata,'{}'::jsonb) into v_metadata
  ...
  if v_family<>'amendment' then
    return v_base||jsonb_build_object(
      'contract','rosetta-independent-structural-validation-v2513c4',
      'document_family',nullif(v_family,'')
    );
  end if;

  v_raw_expected_workflow:=coalesce((v_base->>'expected_workflow_count')::integer,0);
  select count(*)::integer into v_expected from rosetta_v2513.c4_rosetta_v2511_amendment_operations(p_source_text);
  ...
  v_expected_disposition:=rosetta_v2513.c4_rosetta_v2511_amendment_disposition(p_source_text,v_metadata);
  ...
  return v_base||jsonb_build_object(
    'status',v_status,
    'contract','rosetta-independent-structural-validation-v2513c4',
    'document_family','amendment','amendment_format',rosetta_v2513.c4_rosetta_v2511_amendment_format(p_source_text),
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v251_accountability_actor(p_trigger text, p_existing_actor text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO 'pg_catalog'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v251_accountability_kind(p_trigger text)
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v251_validate_independent_structure(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
 v_base:=rosetta_v2513.c4_rosetta_v25_validate_independent_structure(p_extraction_run_id,p_source_text);
 select count(*)::integer into v_typing_mismatch from rosetta_v2513.accountability_route route cross join lateral rosetta_v2513.c4_rosetta_v251_accountability_kind(route.trigger_condition) expected where route.extraction_run_id=p_extraction_run_id and (route.enforcement_type is distinct from expected.enforcement_type or route.enforcement_direction is distinct from expected.enforcement_direction or route.clause_type is distinct from expected.clause_type);
...
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v252_penalty_actor(p_trigger text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
AS $function$ select nullif(btrim(regexp_replace(p_trigger,'(?i)\s+is\s+guilty\M.*$','')),''); $function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v252_validate_independent_structure(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$ select rosetta_v2513.c4_rosetta_v251_validate_independent_structure(p_extraction_run_id,p_source_text)||jsonb_build_object('contract','rosetta-independent-structural-validation-v252'); $function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v253_reconcile_structural_correctness(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
 ...
 for v_route in select route.id,route.trigger_condition,coalesce(nullif(route.actor_source_text,''),nullif(route.enforcement_actor,'')) existing_actor,block.section_number from rosetta_v2513.accountability_route route join rosetta_v2513.hr1_raw_blocks block on block.id=route.source_block_id where route.extraction_run_id=p_extraction_run_id loop
  v_actor:=rosetta_v2513.c4_rosetta_v251_accountability_actor(v_route.trigger_condition,v_route.existing_actor); select * into v_kind from rosetta_v2513.c4_rosetta_v251_accountability_kind(v_route.trigger_condition); if v_kind.enforcement_type='source_stated_penalty_rule' and v_route.trigger_condition ~* '\mis\s+guilty\M' then v_actor:=rosetta_v2513.c4_rosetta_v252_penalty_actor(v_route.trigger_condition); end if;
...
begin v_base:=rosetta_v2513.c4_rosetta_v252_validate_independent_structure(p_extraction_run_id,p_source_text); select count(*)::integer into v_expected from rosetta_v2513.escalation_node node join rosetta_v2513.accountability_route route on route.id=node.accountability_route_id where route.extraction_run_id=p_extraction_run_id; select count(*)::integer into v_actual from rosetta_v2513.rosetta_clause_occurrence occurrence where occurrence.extraction_run_id=p_extraction_run_id; select count(*)::integer into v_binding_mismatch from rosetta_v2513.rosetta_clause_occurrence occurrence join rosetta_v2513.escalation_node node on node.id=occurrence.escalation_node_id where occurrence.extraction_run_id=p_extraction_run_id and (node.accountability_route_id is distinct from occurrence.accountability_route_id or rosetta_v2513.c4_rosetta_normalize_clause_text(node.action_required) is distinct from rosetta_v2513.c4_rosetta_normalize_clause_text(occurrence.source_text)); v_status:=case when coalesce(v_base->>'status','fail')='pass' and v_expected=v_actual and v_binding_mismatch=0 then 'pass' else 'fail' end; return v_base||jsonb_build_object('status',v_status,'contract','rosetta-independent-structural-validation-v253','expected_clause_occurrence_count',v_expected,'actual_clause_occurrence_count',v_actual,'clause_occurrence_binding_mismatch_count',v_binding_mismatch); end;$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v254_canonical_output(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE sql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v254_reconcile_structural_correctness(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
declare v_base jsonb;
begin
  v_base:=rosetta_v2513.c4_rosetta_v253_reconcile_structural_correctness(p_extraction_run_id);
  return v_base||jsonb_build_object('contract','rosetta-structural-reconciliation-v254');
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_actor_source_corrupt(p_actor text)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE
 SET search_path TO 'pg_catalog'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_clause_structurally_sound(p_clause text, p_actor text, p_modal text)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_exact_definition_text(p_source_text text, p_definition_text text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
declare v_source text:=rosetta_v2513.c4_rosetta_v2_normalize_text(rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v25_layout_projection(p_source_text))); v_definition text:=rosetta_v2513.c4_rosetta_v2_normalize_text(p_definition_text); v_position integer;
...
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_is_internal_period(p_value text, p_index integer)
 RETURNS boolean
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
...
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_layout_projection(p_source_text text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  if v_line_label_count >= 3 then
    v_result := rosetta_v2513.c4_rosetta_v25_mask_matches(v_result,'(^|\n)[0-9]{1,3}[.][0-9]{1,3}[ \t]+','n');
  end if;
  ...
  return rosetta_v2513.c4_rosetta_v25_protect_internal_periods(v_result);
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_locate_normalized_text(p_raw_text text, p_needle text)
...
AS $function$
declare
 v_clean_raw text:=rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v25_layout_projection(p_raw_text));
 v_haystack text:=rosetta_v2513.c4_rosetta_v2_normalize_text(v_clean_raw);
 v_needle text:=rosetta_v2513.c4_rosetta_v2_normalize_text(p_needle);
...
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_mask_matches(p_value text, p_pattern text, p_flags text DEFAULT 'n'::text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_modal_and_actor(p_clause text)
...
begin
  v_normalized:=rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v2_normalize_text(p_clause));
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_normative_clauses(p_source_text text)
...
begin
  for v_section in select * from rosetta_v2513.c4_rosetta_v25_section_spans(p_source_text) order by section_ordinal loop
    v_projection := rosetta_v2513.c4_rosetta_v25_layout_projection(v_section.section_text);
    for v_match in select regexp_matches(rosetta_v2513.c4_rosetta_v2_normalize_text(v_projection),'(?i)([^.]*\m(shall not|must not|may not|shall|must|may)\M[^.]*[.])','g') loop
      v_clause := rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v2_normalize_text(v_match[1]));
      select inferred.modal,inferred.actor into v_modal,v_actor from rosetta_v2513.c4_rosetta_v25_modal_and_actor(v_clause) inferred;
      if v_modal is null or v_actor is null then continue; end if;
      if rosetta_v2513.c4_rosetta_v2_is_legislative_finding(v_clause,v_modal) then continue; end if;
      if not rosetta_v2513.c4_rosetta_v25_clause_structurally_sound(v_clause,v_actor,v_modal) then continue; end if;
      v_ordinal:=v_ordinal+1;
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_projected_contains(p_source_text text, p_needle text)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
 select strpos(lower(rosetta_v2513.c4_rosetta_v2_normalize_text(rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v25_layout_projection(p_source_text)))),lower(rosetta_v2513.c4_rosetta_v2_normalize_text(p_needle)))>0;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_protect_internal_periods(p_value text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  for v_index in 2..char_length(p_value) - 1 loop
    if substr(p_value, v_index, 1) = '.' and rosetta_v2513.c4_rosetta_v25_is_internal_period(p_value, v_index) then
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_refresh_object_source_spans(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
  ...
  v_base:=rosetta_v2513.c4_rosetta_v2512_refresh_object_source_spans(p_extraction_run_id, p_source_text);
  ...
  for v_row in
    ...
    from rosetta_v2513.help_entity h join rosetta_v2513.hr1_raw_blocks rb on rb.id=h.source_block_id
  ...
  loop
    ...
    v_source_count:=rosetta_v2513.c4_rosetta_v2512_normalized_occurrence_count(v_block_text,v_row.needle);
    ...
    where h.extraction_run_id=p_extraction_run_id and h.source_block_id=v_row.source_block_id
      and rosetta_v2513.c4_rosetta_v2_normalize_text(h.entity_name)=rosetta_v2513.c4_rosetta_v2_normalize_text(v_row.needle);
    if v_source_count>0 and v_source_count=v_object_count then
      select * into v_loc from rosetta_v2513.c4_rosetta_v2512_locate_normalized_text_occurrence(
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_register_span_repairs(p_extraction_run_id integer)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  select rosetta_v2513.c4_rosetta_blocking_structural_repair_count(p_extraction_run_id) into v_open;
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_section_spans(p_source_text text)
...
AS $function$
declare
  v_projection text := rosetta_v2513.c4_rosetta_v25_layout_projection(p_source_text);
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_unprotect_text(p_value text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_validate_extraction(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE sql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
with expected_workflow as (
  select
    section_number,
    rosetta_v2513.c4_rosetta_v2_normalize_text(clause_text) as clause_text,
    rosetta_v2513.c4_rosetta_v2_normalize_text(actor) as actor,
    lower(modal) as modal
  from rosetta_v2513.c4_rosetta_v25_normative_clauses(p_source_text)
),
actual_workflow as (
  select
    ws.id,
    rosetta_v2513.c4_rosetta_v2_normalize_text(ws.step_name) as clause_text,
    rosetta_v2513.c4_rosetta_v2_normalize_text(ws.actor) as actor,
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_validate_independent_structure(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE plpgsql
 STABLE STRICT
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
  ...
  where pipeline.extraction_run_id=p_extraction_run_id
    and (rosetta_v2513.c4_rosetta_v25_actor_source_corrupt(step.actor) or step.step_name ~* 'REVISOR|ENGROSSMENT|Page No|--\s*[0-9]+\s+of\s+[0-9]+\s*--');
  ...
  where route.extraction_run_id=p_extraction_run_id
    and (rosetta_v2513.c4_rosetta_v25_actor_source_corrupt(coalesce(route.actor_source_text,route.enforcement_actor))
  ...
  select count(*)::integer into v_expected_workflow_count from rosetta_v2513.c4_rosetta_v25_normative_clauses(p_source_text);
  ...
  select rosetta_v2513.c4_rosetta_blocking_structural_repair_count(p_extraction_run_id) into v_blocking_repair_count;
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2_is_legislative_finding(p_clause text, p_modal text)
 RETURNS boolean
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
  select case
    when lower(rosetta_v2513.c4_rosetta_v2_normalize_text(p_clause))
      like '%the legislature finds%'
      or lower(rosetta_v2513.c4_rosetta_v2_normalize_text(p_clause))
      like '%the legislature recognizes%'
      then true
    when lower(p_modal) <> 'may' then false
    else lower(rosetta_v2513.c4_rosetta_v2_normalize_text(p_clause))
      ~ '\m(may offer|may influence|may blur|may create|may lead|may present)\M'
  end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2_modal_and_actor(p_clause text)
 RETURNS TABLE(modal text, actor text)
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
declare
  ...
  v_match text[];
begin
  v_normalized := rosetta_v2513.c4_rosetta_v2_normalize_text(p_clause);
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2_normalize_text(p_value text)
 RETURNS text
 LANGUAGE sql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog'
...
begin
  v_reference_date := case
    when rosetta_v2513.c4_rosetta_v25_validate_reference_date(p_reference_date)
  ...
  select exists(
    ...
    join rosetta_v2513.validation_result independent on independent.extraction_run_id=run.id
      and independent.test_name='independent_structure_v2513c4'
    ...
    join rosetta_v2513.validation_result exact_source on exact_source.extraction_run_id=run.id
      and exact_source.test_name='exact_source_structure_v2513c4'
    ...
    where run.source_document_id=p_source_document_id
      ...
      and run.engine_version='rosetta-v3-deterministic-sql-2.5.13-c4'
      and run.rule_set_version='rosetta-five-layer-structural-correctness-2.5.13-c4'
  ...
  v_receipt:=rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_candidate(
  ...
  return v_receipt||jsonb_build_object(
    ...
    'canonical_producer_contract','rosetta-current-producer-v2513c4',
    'canonical_replay_contract',case when v_was_finalized
      then 'rosetta-finalized-generation-immutable-replay-v2513c4'
      else 'rosetta-final-generation-produced-v2513c4'
    end
  );
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_base(p_source_document_id integer, p_source_text text, p_expected_source_content_hash text, p_source_url text, p_source_version text, p_media_type text DEFAULT 'text/plain'::text, p_source_byte_hash text DEFAULT NULL::text, p_source_provider_hash text DEFAULT NULL::text, p_reference_date date DEFAULT NULL::date, p_text_extractor_version text DEFAULT 'plain-text-1'::text, p_source_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
AS $function$
declare
  v_engine_version constant text := 'rosetta-v3-deterministic-sql-2.5.13-c4';
  v_rule_set_version constant text := 'rosetta-five-layer-structural-correctness-2.5.13-c4';
...
begin
  ...
  insert into rosetta_v2513.extraction_run_config (
    ...
    'cfg-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash,
    v_run_id,
    1.00,
    true,
    1.00,
  ...
  if v_is_incomplete then
    ...
    insert into rosetta_v2513.extraction_manifest (
      ...
      'manifest-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash,
      v_run_id,
      p_source_document_id,
      v_corpus_id,
      1,
    ...
    insert into rosetta_v2513.validation_result (
      ...
      'vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-source-complete',
      v_run_id,
      'source_complete',
      'fail',
      1,
  ...
  v_flat := rosetta_v2513.c4_rosetta_v2_normalize_text(p_source_text);
  ...
  v_block_id := 'blk-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-root';
  ...
  for v_section in
    select *
    from rosetta_v2513.c4_rosetta_v25_section_spans(p_source_text)
    order by section_ordinal
  loop
    v_section_number := v_section.section_number;
    v_section_flat := rosetta_v2513.c4_rosetta_v2_normalize_text(rosetta_v2513.c4_rosetta_v25_layout_projection(v_section.section_text));
    v_section_hash := encode(
      digest(convert_to(v_section.section_text, 'UTF8'), 'sha256'),
      'hex'
    );
    v_section_block_id :=
      'blk-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
    ...
    v_pipeline_id :=
      'wp-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
      lpad(v_section.section_ordinal::text, 4, '0');
    ...
    loop
      ...
      insert into rosetta_v2513.help_entity (
        ...
        'he-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
    ...
    for v_clause_row in
      select *
      from rosetta_v2513.c4_rosetta_v25_normative_clauses(v_section.section_text)
    ...
    loop
      ...
      insert into rosetta_v2513.workflow_step (
        ...
        'ws-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
      ...
      then
        ...
        insert into rosetta_v2513.accountability_route (
          ...
          'ar-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
        ...
        insert into rosetta_v2513.escalation_node (
          ...
          'en-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
            lpad(v_accountability_count::text, 4, '0'),
          'ar-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
            lpad(v_accountability_count::text, 4, '0'),
          1,
    ...
    loop
      ...
      then
        ...
        from rosetta_v2513.c4_rosetta_v2_modal_and_actor(v_clause) inferred;
        ...
        insert into rosetta_v2513.entity_override (
          ...
          'ov-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
    ...
    loop
      ...
      insert into rosetta_v2513.term_definition (
        ...
        'td-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
        ...
        btrim(v_match[1]),
        v_section_number,
        rosetta_v2513.c4_rosetta_v25_unprotect_text(btrim(v_match[2] || ' ' || v_match[3])),
        'technical',
        1.00,
        'confirmed'
      );
    ...
    select
      'lc-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-' ||
  ...
  v_structural_validation := rosetta_v2513.c4_rosetta_v25_validate_extraction(v_run_id, p_source_text);
  ...
  insert into rosetta_v2513.validation_result (
    ...
    'vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-structural-correctness',
    v_run_id,
  ...
  insert into rosetta_v2513.validation_result (
    ...
    ('vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-source-hash', v_run_id, 'source_hash_verified', 'pass', 0,
      jsonb_build_object('source_content_hash', v_source_content_hash)),
    ('vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-source-bytes', v_run_id, 'source_bytes_receipted', 'pass', 0,
      jsonb_build_object('source_byte_hash', p_source_byte_hash, 'media_type', p_media_type)),
    ('vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-coverage', v_run_id, 'five_layer_coverage', 'pass', 0,
      jsonb_build_object('coverage', v_coverage)),
    ('vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-no-pending', v_run_id, 'no_pending_coverage', 'pass', 0,
      jsonb_build_object('coverage', v_coverage)),
    ('vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-source-bound', v_run_id, 'canonical_rows_source_bound', 'pass', 0,
      jsonb_build_object('source_block_id', v_block_id)),
    ('vr-v2513c4-' || v_source_identity_hash || '-' || v_configuration_hash || '-output-hash', v_run_id, 'output_hash_verified', 'pass', 0,
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_candidate(p_source_document_id integer, p_source_text text, p_expected_source_content_hash text, p_source_url text, p_source_version text, p_media_type text DEFAULT 'text/plain'::text, p_source_byte_hash text DEFAULT NULL::text, p_source_provider_hash text DEFAULT NULL::text, p_reference_date date DEFAULT NULL::date, p_text_extractor_version text DEFAULT 'plain-text-1'::text, p_source_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET statement_timeout TO '180s'
...
begin
  ...
  where engine_version='rosetta-v3-deterministic-sql-2.5.13-c4'
    and rule_set_version='rosetta-five-layer-structural-correctness-2.5.13-c4'
    and is_active=true;
  if v_manifest_hash is null then raise exception 'rosetta_v2513c4_active_manifest_missing'; end if;

  v_base:=rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_base(
  ...
  if not v_finalized then
    v_receipt:=rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_candidate_base(
      p_source_document_id,p_source_text,p_expected_source_content_hash,p_source_url,p_source_version,p_media_type,
      p_source_byte_hash,p_source_provider_hash,p_reference_date,p_text_extractor_version,p_source_metadata
    );
    ...
    v_receipt:=v_base||jsonb_build_object(
      ...
      'replay_contract','rosetta-finalized-generation-immutable-replay-v2513c4'
    );
  end if;
  ...
  v_coverage:=rosetta_v2513.c4_rosetta_v2511_final_coverage(v_run_id);
  ...
  where extraction_run_id=v_run_id and test_name='independent_structure_v2513c4';
  ...
  return v_receipt||jsonb_build_object(
    ...
    'independent_structure_v2513c4',v_independent
  );
end;
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_candidate_base(p_source_document_id integer, p_source_text text, p_expected_source_content_hash text, p_source_url text, p_source_version text, p_media_type text DEFAULT 'text/plain'::text, p_source_byte_hash text DEFAULT NULL::text, p_source_provider_hash text DEFAULT NULL::text, p_reference_date date DEFAULT NULL::date, p_text_extractor_version text DEFAULT 'plain-text-1'::text, p_source_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET statement_timeout TO '120s'
...
begin
  ...
  where engine_version='rosetta-v3-deterministic-sql-2.5.13-c4' and rule_set_version='rosetta-five-layer-structural-correctness-2.5.13-c4';

  v_receipt:=rosetta_v2513.c4_run_rosetta_v3_extraction_v2511_base(
    p_source_document_id,p_source_text,p_expected_source_content_hash,p_source_url,p_source_version,p_media_type,
    p_source_byte_hash,p_source_provider_hash,p_reference_date,p_text_extractor_version,p_source_metadata
  );
  ...
  begin
    v_receipt:=rosetta_v2513.c4_rosetta_v2511_finalize_extraction(v_run_id,p_source_text,coalesce(p_source_metadata,'{}'::jsonb),v_receipt);
    v_reclassification:=rosetta_v2513.c4_rosetta_v2511_reclassify_amendment_structure(v_run_id,p_source_text,coalesce(p_source_metadata,'{}'::jsonb));
    v_span_receipt:=rosetta_v2513.c4_rosetta_v25_refresh_object_source_spans(v_run_id,p_source_text);
    v_span_repairs:=rosetta_v2513.c4_rosetta_v25_register_span_repairs(v_run_id);
    v_reconciliation:=rosetta_v2513.c4_rosetta_v2511_reconcile_structural_correctness(v_run_id);
    v_coverage:=rosetta_v2513.c4_rosetta_v2511_refresh_final_coverage_receipts(v_run_id);
    v_self_check:=rosetta_v2513.c4_rosetta_v2511_validate_extraction(v_run_id,p_source_text);
    v_independent:=rosetta_v2513.c4_rosetta_v2511_validate_independent_structure(v_run_id,p_source_text);
  exception when others then
    v_error:=left(sqlerrm,240);
    update rosetta_v2513.extraction_run set run_status='failed',admissibility_state='rejected',failure_code='rosetta_v2513c4_post_base_failure',completed_at=clock_timestamp() where id=v_run_id;
    update rosetta_v2513.extraction_manifest set status='failed',admissibility_state='rejected',validation_results=coalesce(validation_results,'{}'::jsonb)||jsonb_build_object('rosetta_v2513c4_post_base_failure',jsonb_build_object('message',v_error)) where extraction_run_id=v_run_id;
    return v_receipt||jsonb_build_object('engine_version','rosetta-v3-deterministic-sql-2.5.13-c4','rule_set_version','rosetta-five-layer-structural-correctness-2.5.13-c4','rule_manifest_hash',v_manifest_hash,'run_status','failed','admissibility_state','rejected','failure_code','rosetta_v2513c4_post_base_failure');
  end;
  ...
  insert into rosetta_v2513.validation_result(id,extraction_run_id,test_name,test_result,failure_count,details)
  values('vr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-structural-correctness',v_run_id,'structural_correctness_v2',case when v_self_check->>'status'='pass' then 'pass' else 'fail' end,case when v_self_check->>'status'='pass' then 0 else 1 end,v_self_check)
  on conflict(extraction_run_id,test_name) do update set test_result=excluded.test_result,failure_count=excluded.failure_count,details=excluded.details,executed_at=now();

  insert into rosetta_v2513.validation_result(id,extraction_run_id,test_name,test_result,failure_count,details)
  values('vr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-independent-structure',v_run_id,'independent_structure_v2513c4',case when v_independent->>'status'='pass' then 'pass' else 'fail' end,case when v_independent->>'status'='pass' then 0 else 1 end,v_independent)
  on conflict(extraction_run_id,test_name) do update set test_result=excluded.test_result,failure_count=excluded.failure_count,details=excluded.details,executed_at=now();

  insert into rosetta_v2513.validation_result(id,extraction_run_id,test_name,test_result,failure_count,details)
  values('vr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-exact-source-structure',v_run_id,'exact_source_structure_v2513c4',case when v_exact->>'status'='pass' then 'pass' else 'fail' end,case when v_exact->>'status'='pass' then 0 else 1 end,v_exact)
  on conflict(extraction_run_id,test_name) do update set test_result=excluded.test_result,failure_count=excluded.failure_count,details=excluded.details,executed_at=now();
  ...
  v_pass:=coalesce(v_coverage->>'status','fail')='pass'
    ...
    and rosetta_v2513.c4_rosetta_blocking_structural_repair_count(v_run_id)=0;

  if not v_pass then
    update rosetta_v2513.extraction_run set run_status='failed',admissibility_state='rejected',failure_code='rosetta_v2513c4_final_validation_failed',completed_at=clock_timestamp() where id=v_run_id;
    update rosetta_v2513.extraction_manifest set status='failed',admissibility_state='rejected',validation_results=coalesce(validation_results,'{}'::jsonb)||jsonb_build_object('structural_reclassification_v2513c4',v_reclassification,'structural_reconciliation_v2513c4',v_reconciliation,'object_source_spans_v25',v_span_receipt,'span_repair_registration_v25',v_span_repairs,'final_five_layer_coverage_v2513c4',v_coverage,'structural_correctness_v2',v_self_check,'independent_structure_v2513c4',v_independent,'exact_source_structure_v2513c4',v_exact) where extraction_run_id=v_run_id;
    return v_receipt||jsonb_build_object('engine_version','rosetta-v3-deterministic-sql-2.5.13-c4','rule_set_version','rosetta-five-layer-structural-correctness-2.5.13-c4','rule_manifest_hash',v_manifest_hash,'run_status','failed','admissibility_state','rejected','failure_code','rosetta_v2513c4_final_validation_failed','independent_structure_v2513c4',v_independent);
  end if;

  v_output:=rosetta_v2513.c4_rosetta_v2511_canonical_output(v_run_id);
  if v_output is null then raise exception 'rosetta_v2513c4_final_canonical_output_unavailable'; end if;
  v_output_hash:=encode(digest(convert_to(v_output::text,'UTF8'),'sha256'),'hex');
  ...
  insert into rosetta_v2513.validation_result(id,extraction_run_id,test_name,test_result,failure_count,details)
  values('vr-v2513c4-'||v_run.source_identity_hash||'-'||v_run.configuration_hash||'-output-hash',v_run_id,'output_hash_verified','pass',0,jsonb_build_object('output_content_hash',v_output_hash,'contract','rosetta-final-output-hash-v2513c4'))
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v25_validate_reference_date(
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2512_layout_projection(p_source_text text)
 RETURNS text
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
...
begin
  ...
  if v_trailing_line_number_count >= 12 then
    v_result := rosetta_v2513.c4_rosetta_v25_mask_matches(v_result,'\t[0-9]{1,3}[ \t]*(\n|$)','n');
  end if;
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2512_locate_normalized_text_occurrence(p_raw_text text, p_needle text, p_occurrence integer)
 RETURNS TABLE(source_offset_start integer, source_offset_end integer, span_status text)
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
declare
 v_clean_raw text:=rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v2512_layout_projection(p_raw_text)); v_haystack text:=rosetta_v2513.c4_rosetta_v2_normalize_text(v_clean_raw); v_needle text:=rosetta_v2513.c4_rosetta_v2_normalize_text(p_needle); v_search integer:=1; v_relative integer; v_start_norm integer:=0; v_end_norm integer; v_iteration integer; v_index integer; v_char text; v_norm_pos integer:=0; v_seen_nonspace boolean:=false; v_pending_space boolean:=false; v_space_raw_start integer:=null; v_raw_start integer:=null; v_raw_end integer:=null;
begin
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2512_normalized_occurrence_count(p_raw_text text, p_needle text)
 RETURNS integer
 LANGUAGE plpgsql
 IMMUTABLE STRICT
 SET search_path TO 'pg_catalog', 'public'
AS $function$
declare v_haystack text:=lower(rosetta_v2513.c4_rosetta_v2_normalize_text(rosetta_v2513.c4_rosetta_v25_unprotect_text(rosetta_v2513.c4_rosetta_v2512_layout_projection(p_raw_text)))); v_needle text:=lower(rosetta_v2513.c4_rosetta_v2_normalize_text(p_needle)); v_cursor integer:=1; v_relative integer; v_count integer:=0;
...
$function$;
CREATE OR REPLACE FUNCTION rosetta_v2513.c4_rosetta_v2512_refresh_object_source_spans(p_extraction_run_id integer, p_source_text text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'extensions'
...
begin
 ...
 loop
  v_needle:=v_row.needle; v_definition_only:=false; v_block_text:=substr(p_source_text,v_row.block_start+1,v_row.block_end-v_row.block_start); v_source_count:=rosetta_v2513.c4_rosetta_v2512_normalized_occurrence_count(v_block_text,v_needle);
  if v_row.object_type='term_definition' and v_source_count=0 then select td.definition_text into v_needle from rosetta_v2513.term_definition td where td.id=v_row.object_id; v_definition_only:=true; v_source_count:=rosetta_v2513.c4_rosetta_v2512_normalized_occurrence_count(v_block_text,v_needle); end if;
  if v_row.object_type='workflow_step' then select count(*)::integer,count(*) filter(where ws.id<=v_row.object_id)::integer into v_object_count,v_object_ordinal from rosetta_v2513.workflow_step ws join rosetta_v2513.workflow_pipeline wp on wp.id=ws.workflow_pipeline_id where wp.extraction_run_id=p_extraction_run_id and wp.source_block_id=v_row.source_block_id and rosetta_v2513.c4_rosetta_v2_normalize_text(ws.step_name)=rosetta_v2513.c4_rosetta_v2_normalize_text(v_needle);
  elsif v_row.object_type='accountability_route' then select count(*)::integer,count(*) filter(where ar.id<=v_row.object_id)::integer into v_object_count,v_object_ordinal from rosetta_v2513.accountability_route ar where ar.extraction_run_id=p_extraction_run_id and ar.source_block_id=v_row.source_block_id and rosetta_v2513.c4_rosetta_v2_normalize_text(ar.trigger_condition)=rosetta_v2513.c4_rosetta_v2_normalize_text(v_needle);
  elsif v_row.object_type='entity_override' then select count(*)::integer,count(*) filter(where eo.id<=v_row.object_id)::integer into v_object_count,v_object_ordinal from rosetta_v2513.entity_override eo where eo.extraction_run_id=p_extraction_run_id and eo.source_block_id=v_row.source_block_id and rosetta_v2513.c4_rosetta_v2_normalize_text(eo.override_scope)=rosetta_v2513.c4_rosetta_v2_normalize_text(v_needle);
  else
   if v_definition_only then select count(*)::integer,count(*) filter(where td.id<=v_row.object_id)::integer into v_object_count,v_object_ordinal from rosetta_v2513.term_definition td where td.extraction_run_id=p_extraction_run_id and td.source_block_id=v_row.source_block_id and rosetta_v2513.c4_rosetta_v2_normalize_text(td.definition_text)=rosetta_v2513.c4_rosetta_v2_normalize_text(v_needle);
   else select count(*)::integer,count(*) filter(where td.id<=v_row.object_id)::integer into v_object_count,v_object_ordinal from rosetta_v2513.term_definition td where td.extraction_run_id=p_extraction_run_id and td.source_block_id=v_row.source_block_id and rosetta_v2513.c4_rosetta_v2_normalize_text('"'||td.defined_term||'" '||td.definition_text)=rosetta_v2513.c4_rosetta_v2_normalize_text(v_needle); end if;
  end if;
  if v_source_count>0 and v_source_count=v_object_count then select * into v_loc from rosetta_v2513.c4_rosetta_v2512_locate_normalized_text_occurrence(v_block_text,v_needle,v_object_ordinal); v_status:=v_loc.span_status;
  elsif v_source_count>0 then select * into v_loc from rosetta_v2513.c4_rosetta_v2512_locate_normalized_text_occurrence(v_block_text,v_needle,1); v_status:='ambiguous';
  else v_status:='unresolved'; end if;
  ...
  values(v_row.object_type,v_row.object_id,p_extraction_run_id,v_row.source_document_id,v_row.source_block_id,v_absolute_start,v_absolute_end,v_raw_text,v_needle,case when v_raw_text is null then null else encode(digest(convert_to(v_raw_text,'UTF8'),'sha256'),'hex') end,'rosetta-layout-projection-v2513c4',v_status)
 ...
 return jsonb_build_object('contract','rosetta-object-source-span-v2513c4','extraction_run_id',p_extraction_run_id,'resolved',v_resolved,'ambiguous',v_ambiguous,'unresolved',v_unresolved,'occurrence_rule','resolve_only_when_source_occurrence_count_equals_object_count');
end;
$function$;
insert into rosetta_v2513.extraction_rule_manifest
  (engine_version, rule_set_version, manifest_hash, manifest_json, is_active)
values ('rosetta-v3-deterministic-sql-2.5.13-c4', 'rosetta-five-layer-structural-correctness-2.5.13-c4',
        'ee723682dfcf6029c1c855a06ccdb936d1bd0a288ad92a7ca03ff61a151fddb0', $manifest${"changes":["C4 occurrence-aware span binding and credible reference-date gate"],"closure_namespace":"rosetta_v2513","closure_prefix":"c4_","control_identity":"rosetta-v3-deterministic-sql-2.5.11","engine_version":"rosetta-v3-deterministic-sql-2.5.13-c4","lane":"c4","publication":"structurally disabled: no publication view, no registry row, no publishable-run path references this namespace","rule_set_version":"rosetta-five-layer-structural-correctness-2.5.13-c4","title":"C4 occurrence-aware span binding and credible reference-date gate"}$manifest$::jsonb, true);

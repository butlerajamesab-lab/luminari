...
begin
  ...
  select count(*) into v_candidate_ambiguity
  from rosetta_replay.sealed_corpus_member m
  join rosetta_replay.replay_run_binding b on b.source_registry_id=m.source_registry_id
    and b.source_content_id=m.source_content_id and b.source_content_hash=m.source_content_hash
    and b.engine_version=p_engine_version and b.rule_set_version=p_rule_set_version
  ...
  select count(*) into v_control_ambiguity
  from rosetta_replay.sealed_corpus_member m
  cross join lateral rosetta_replay.run_object_field_snapshot(m.control_run_id) s
  where m.manifest_id=p_manifest_id and m.prior_output_state='admissible'
    and s.field='span_status' and coalesce(s.field_value,'unresolved') in ('ambiguous','unresolved');
  if v_candidate_ambiguity>v_control_ambiguity then
    raise exception 'gate G7: candidate ambiguity % exceeds prior baseline %',v_candidate_ambiguity,v_control_ambiguity
      using errcode='P1G17';
  end if;

...
export function strip_sql_noncode(sql_text) {
...
export const strip_sql_comments = strip_sql_noncode;
...
export function extract_full_substrate_targets(sql_text) {
  ...
  const found = new Set();
  let match;
  const executable_sql = strip_sql_noncode(sql_text);
...
export function assert_full_substrate_sql_safe(sql_text, options = {}) {
  ...
  if (FORBIDDEN_FULL_SUBSTRATE_SQL.test(strip_sql_noncode(sql_text))) {

...
function detectParser(row) {
  ...
  const targetHint = normalize(row.target_hint);
  const importStatus = normalize(row.import_status);
  const hasRawText = typeof row.raw_text === "string" && row.raw_text.trim().length > 0;
  ...
  if (ext === ".docx" || contentType.includes("wordprocessingml") || targetHint === "state_enriched_registry_docx_review") {
    if (importStatus === "docx_extraction_failed") return { parser: "docx", parserStatus: "available_for_review", reason: "DOCX extraction rail ran but failed; route to review with extraction error." };
    if (importStatus === "docx_empty_text_review") return { parser: "docx", parserStatus: "available_for_review", reason: "DOCX extraction rail ran but produced empty text; route to review." };
    if (hasRawText || importStatus === "pending_docx_normalization") return { parser: "docx", parserStatus: "available_for_review", reason: "DOCX raw text exists and is ready for normalization/review before gate routing." };
    return { parser: "docx", parserStatus: "available_for_review", reason: "DOCX extraction rail is available; run scripts/extract-docx-corpus-queue.mjs before normalization." };
  }

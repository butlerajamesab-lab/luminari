...
const canonicalAuthHook = readFileSync('client/src/core/hooks/useAuth.ts', 'utf8');
const privateQueryCache = readFileSync('client/src/core/privateQueryCache.ts', 'utf8');
...
function readJavaScriptBundles(directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) bundles.push(...readJavaScriptBundles(path));
    else if (entry.isFile() && entry.name.endsWith('.js')) bundles.push([path, readFileSync(path, 'utf8')]);
  }
  return bundles;
}

const runtimeBundles = existsSync('dist') ? readJavaScriptBundles('dist') : [];
...
const syntheticInspectionMarkers = [
  'LIGHTHOUSE_INSPECTION_MODE',
  'x-lighthouse-inspection-mode',
  'inspection_user',
  'Inspection User',
...
const queryCacheStart = clientEntry.indexOf('queryClient.getQueryCache().subscribe');
const mutationCacheStart = clientEntry.indexOf('queryClient.getMutationCache().subscribe');
const queryCacheSection = clientEntry.slice(queryCacheStart, mutationCacheStart);
const mutationCacheSection = clientEntry.slice(mutationCacheStart, clientEntry.indexOf('// Helper: get a fresh Supabase session token'));
if (queryCacheStart < 0 || mutationCacheStart < 0) fail('client authentication error boundaries are missing');
if (queryCacheSection.includes('redirectToLoginIfUnauthorized(error)')) fail('private query failures must not obstruct public browsing');
if (!mutationCacheSection.includes('redirectToLoginIfUnauthorized(error)')) fail('unauthorized mutations must return users to canonical Supabase sign-in');
if (!canonicalAuthHook.includes('clearPrivateQueryCache(queryClient)')) fail('private client query data must be purged on sign-out');
if (!canonicalAuthHook.includes('event === "SIGNED_OUT"')) fail('Supabase sign-out must trigger private query cache clearing');
const privateQueryReset = privateQueryCache.indexOf('query.reset()');
const privateQueryClear = privateQueryCache.indexOf('queryClient.clear()');
if (privateQueryReset < 0 || privateQueryClear < 0 || privateQueryReset >= privateQueryClear) fail('active private query observers must be reset before their cache entries are cleared');
if (privateQueryCache.includes('queryClient.resetQueries(')) fail('private query purge must not refetch active queries after sign-out');
